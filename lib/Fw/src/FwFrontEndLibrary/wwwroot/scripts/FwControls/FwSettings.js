...
//---------------------------------------------------------------------------------------------- 
FwSettings.newRow = function ($body, $form, $control, apiurl, $modulecontainer, moduleName, $modules) {
    var rowId, newRowHtml = [];
    ...
    $body.on('click', '.save-new-row', function (e) {
        ...
        save = $form.find('.btn')
        FwSettings.saveForm(window[moduleName + 'Controller'].Module, $form, false, '', $control, [], rowId, moduleName, function () {
            $body.find('.new-row').hide();
            $body.empty();
            FwSettings.getRows($body, $control, apiurl, $control.find('#' + moduleName), moduleName);
            FwModule.closeForm($form);
        });
    });
}
//---------------------------------------------------------------------------------------------- 
FwSettings.renderModuleHtml = function ($control, title, moduleName, color, description) {
    ...
    $settingsPageModules.on('click', '.btn', function (e) {
        ...
        FwSettings.newRow($body, $form, $control, apiurl, $modulecontainer, moduleName, $settingsPageModules);
    });
