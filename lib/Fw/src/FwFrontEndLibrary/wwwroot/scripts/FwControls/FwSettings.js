...
FwSettings.renderRuntimeHtml = function ($control) {
    ...
    html.push('    <input type="text" id="moduleSearch" class="form-control" placeholder="Field...">');
...
FwSettings.renderModuleHtml = function ($control, title, description, color) {
    ...
    html.push('        </div>'); 
    html.push('      </h4>');
    html.push('      <small id="description">' + description + '</small>')
    html.push('    </div>');
    ...
    $settingsPageModules.on('click', '.new-row', function (e) {
        var $this, moduleName, $modulecontainer, apiurl, $body, $form, browseKeys = [], rowId, formKeys = [], newRowHtml = [];
        ...
        var getRows = function () {
            FwAppData.apiMethod(true, 'GET', applicationConfig.appbaseurl + applicationConfig.appvirtualdirectory + apiurl, null, null, function onSuccess(response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    for (var j = 0; j < browseKeys.length; j++) {
                        if (browseKeys[j] === 'Inactive') {
                            browsKeys.splice(j, 1);
                        }
                        if (browseKeys.length === 1) {
                            html.push('      <label style="width:25%">' + browseKeys[j] + '</label>');
                        ...
                        if (browseKeys.length === 4) {
                            html.push('      <label style="width:10%">' + browseKeys[j] + '</label>');
                            html.push('      <label style="width:10%">' + response[i][browseKeys[j]] + '</label>');
                        }
                        if (browseKeys.length === 5) {
                            html.push('      <label style="width:8%">' + browseKeys[j] + '</label>');
                            html.push('      <label style="width:8%">' + response[i][browseKeys[j]] + '</label>');
                        }
                    }
                    html.push('      <div class="pull-right save"><i class="material-icons">save</i>Save</div>');
                    html.push('    </div>');
