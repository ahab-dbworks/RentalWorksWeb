...
FwModule.openSubModuleTab = function($browse, $form) {
    ...
    if (typeof window[$form.attr('data-controller')] === 'object')
    {
...
FwModule.addFormMenu = function($form) {
    var controller, $menu, $save, $edit, $delete,
      ...
      nodeBrowse, nodeBrowseMenuBar, nodeBrowseNewButton, $newmenubarbutton;
    ...
    if (nodeBrowse !== null) {
        nodeBrowseMenuBar = FwApplicationTree.getChildByType(nodeBrowse, 'MenuBar');
        if (nodeBrowseMenuBar !== null) {
            nodeBrowseNewButton = FwApplicationTree.getChildByType(nodeBrowseMenuBar, 'NewMenuBarButton');
            if (nodeBrowseNewButton !== null) {
                $newmenubarbutton = FwMenu.addStandardBtn($menu, "New");
                $newmenubarbutton.attr('data-type', 'NewMenuBarButton');
                $newmenubarbutton.on('click', function () {
                    var $form, controller, $browse, issubmodule;
                    try {
                        $form = jQuery(this).closest('.fwform');
                        controller = $form.attr('data-controller');
                        issubmodule = $form.closest('.tabpage').hasClass('submodule');
                        if (typeof window[controller] !== 'object') throw 'Missing javascript module: ' + controller;
                        if (typeof window[controller].openForm !== 'function') throw 'Missing javascript function: ' + controller + '.openForm';
                        $form = window[controller].openForm('NEW');
                        if (!issubmodule) {
...
FwModule.closeForm = function($form, $tab, navigationpath, afterCloseForm, closeParent) {
    ...
    if (hassubmodule) {
        ...
        FwModule.closeForm($submoduleform, $submoduletab, navigationpath, '', true);
    } else {
        if (ismodified === 'true') {
