...
//----------------------------------------------------------------------------------------------
//FwModule.closeTabs = function() {
//    var $moduleTabs, $scroller;
//    $moduleTabs = jQuery('#moduleMaster-body').find('#moduletabs');
//    $scroller   = jQuery('#moduleMaster-body').find('#moduletabs').find('.fwscroller');
//    FwScroller.screenunload($scroller);
//    FwTabs.closeTabs($moduleTabs);
//};
//----------------------------------------------------------------------------------------------
FwModule.openModuleTab = function($object, caption, tabHasClose, tabType, setTabActive) {
    var $tabControl, newtabids, hasscheduler, $schedulers, $fwcontrols, controller;
    ...
    if ($object.hasClass('fwbrowse')) {
        if (typeof window[$object.attr('data-controller')] === 'object')
        {
...
FwModule.openSubModuleTab = function($browse, $form) {
    var $parentform, $submoduleformtab, $submodulebrowsetab;

    $parentform = $browse.closest('.fwform');

    FwModule.openFormTab($parentform, $form, 'New ' + $form.attr('data-caption'), true, 'FORM', true);
    $submoduleformtab   = jQuery('#' + $form.closest('.tabpage').attr('data-tabid'));
    $submodulebrowsetab = jQuery('#' + $browse.closest('.tabpage').attr('data-tabid'));

    $submoduleformtab.css('margin-left', '10px').remove();
    $submodulebrowsetab.after($submoduleformtab);
};
//----------------------------------------------------------------------------------------------
            ...
            if (nodeBrowseMenuBar !== null) {
                if (nodeBrowseMenuBar.properties.visible === 'T') {
                    ...
                    for (var menubaritemno = 0; menubaritemno < nodeBrowseMenuBar.children.length; menubaritemno++) {
                        nodeMenuBarItem = nodeBrowseMenuBar.children[menubaritemno];
                                                    ...
                                                    switch(FwApplicationTree.getNodeType(nodeSubMenuItem))
                                                    {
                                                        case 'SubMenuItem':
                                                            ...
                                                            hasClickEvent = ((typeof controller === 'string') && 
                                                                                (controller.length > 0) && 
                                                                                (typeof FwApplicationTree.clickEvents === 'object') && 
...
FwModule.loadForm = function(module, $form) {
    var $tabpage, fields, ids, request;
    $tabpage = $form.parent();
    ...
    FwServices.module.method(request, module, 'Load', $form,
        // onSuccess
        function(response) {
            var controller, issubmodule, $fwchargefields, $formfields;
            try {
                var controller = window[module + 'Controller'];
                if (typeof controller === 'undefined') {
                    throw module + 'Controller is not defined.'
                }
                if (typeof controller.apiurl === 'undefined' && typeof response.tables !== 'object') {
                ...
                if (typeof controller.apiurl !== 'undefined') {
                    ...
                    //var tabname = (typeof response.tabname === 'string') ? response.tabname : (typeof response.Caption === 'string') ? response.Caption: 'Unknown';
                    ...
                    if (!issubmodule) {
                        jQuery('#' + $form.parent().attr('data-tabid')).find('.caption').html(tabname);
                    } else {
                        jQuery('#' + $form.parent().attr('data-tabid')).find('.form-caption').html(tabname);
                    }
...
//----------------------------------------------------------------------------------------------
FwModule.saveForm = function(module, $form, closetab, navigationpath) {
        ...
        FwServices.module.method(request, module, 'Save', $form, function(response) {
            ...
            if (typeof controller.apiurl !== 'undefined') {
                if (!closetab) {
                    ...
                    issubmodule = $form.parent().hasClass('submodule');
                    if (!issubmodule) {
                        //jQuery('#' + $form.parent().attr('data-tabid')).find('.caption').html(response.RecordTitle);
                        jQuery('#' + $form.parent().attr('data-tabid')).find('.caption').html(tabname);
                    } else {
                        //jQuery('#' + $form.parent().attr('data-tabid')).find('.form-caption').html(response.RecordTitle);
                        jQuery('#' + $form.parent().attr('data-tabid')).find('.form-caption').html(tabname);
                    }
...
FwModule.deleteRecord = function(module, $control) {
    ...
    try {
        ...
        if ($selectedRow.length > 0) {
            ids           = {};
            $confirmation = FwConfirmation.renderConfirmation('Delete Record', 'Are you sure you want to delete this record?');
            $yes          = FwConfirmation.addButton($confirmation, 'Yes');
            ...
            if (nodeFormMenuBar !== null) {
                if (nodeFormMenuBar.properties.visible === 'T') {
                    ...
                    for (var menubaritemno = 0; menubaritemno < nodeFormMenuBar.children.length; menubaritemno++) {
                        nodeMenuBarItem = nodeFormMenuBar.children[menubaritemno];
                        if (nodeMenuBarItem.properties.visible === 'T') {
...
//----------------------------------------------------------------------------------------------
FwModule.closeForm = function($form, $tab, navigationpath, afterCloseForm) {
    var $tabcontrol, ismodified, isSubModule, $confirmation, $save, $dontsave, $cancel, tabname;
    $tabcontrol = $tab.closest('.fwtabs');
    ismodified  = $form.attr('data-modified');
    isSubModule = $tab.hasClass('submodule');

    if (ismodified === 'true') {
        tabname = ((isSubModule) ? $tab.find('.form-caption').html() : $tab.find('.caption').html());
        ...
        $save.on('click', function() {
            ...
            controller = $form.attr('data-controller');
            if (typeof window[controller] !== 'object') throw 'Missing javascript module controller: ' + controller;
            if (typeof window[controller]['saveForm'] === 'function') {
                window[controller]['saveForm']($form, true, navigationpath);
            }
...
FwModule.closeFormTab = function($tab) {
    ...
    isSubModule = $tab.hasClass('submodule');
    if (isSubModule) {
        $tab.find('.submodule-caption').hide();
        $tab.find('.form-caption').html('');
        $tab.find('.modified').html('');
        $form   = $tabpage.find('.fwform');
        $form.remove();
        $browse = $tabpage.find('.fwbrowse');
        $browse.show();
        FwBrowse.databind($browse);
    } else {
...
FwModule.getFormFields = function($form, getAllFieldsOverride) {
    ...
    $fwformfields.each(function(index, element) {
        ...
        if (typeof dataField === 'undefined') {
            ...
            console.log('On Form: "' + formCaption + ' ", the attribute data-datafield is required on the fwformfield with the following html: ' + jQuery('div').append($formfield).html());
