FwBrowseColumn_combobox = {};
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.databindfield = function($browse, $field, dt, dtRow, field, $tr) {
    var displayFieldValue = dtRow[dt.ColumnIndex[field.browsedisplayfield]];
    $field.attr('data-originaltext', displayFieldValue);
};
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.getFieldUniqueId = function($browse, $tr, $field, uniqueid, originalvalue) {
    if ($tr.hasClass('editmode')) {
        uniqueid.value = $field.find('input.value').val();
    }
};
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.getFieldValue = function($browse, $tr, $field, field, originalvalue) {
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.setFieldEditMode = function($browse, $field, $tr, html) {
    ...
    FwBrowseColumn_combobox.initControl($field);
};
//---------------------------------------------------------------------------------
// begin shared code with browse column
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.initControl = function($control) {
    ...
    $validationbrowse.data('ondatabind', function(request) {
        ...
        if ((typeof controller === 'string') && (typeof window[controller] === 'object') && (typeof window[controller][formbeforevalidate] === 'function')) {
            if ($object.attr('data-type') === 'Grid') {
                controller[formbeforevalidate]($validationbrowse, $object.closest('.fwform'), request);
            } else {
                controller[formbeforevalidate]($validationbrowse, $object, request);
            }
        }
    });
        ...
        FwServices.validation.method(request, request.module, 'Browse', $control, function(response) {
            try {
                if (typeof controller.apiurl !== 'undefined') {
                    FwBrowseColumn_combobox.databindcallback($control, $validationbrowse, response, validationName, $valuefield, $btnvalidate, $validationbrowse, controller);
                } else {
                    FwBrowseColumn_combobox.databindcallback($control, $validationbrowse, response.browse, validationName, $valuefield, $btnvalidate, $validationbrowse, controller);
                }
            } catch (ex) {
                FwFunc.showError(ex);
            }
            ...
            try {
                ...
                if ($control.data('dropdown') !== null) {
                    if (e.which === 38) { // Up Arrow
                        FwBrowseColumn_combobox.highlightPrevRow($control);
                        e.preventDefault();
                        return;
                    }
                    if (e.which === 40) { // Down Arrow
                        FwBrowseColumn_combobox.highlightNextRow($control);
                        e.preventDefault();
                        return;
                    }
                    if(e.which === 13) { //Enter Key
                        $row = FwBrowseColumn_combobox.getHighlightedRow($control);
                        FwBrowseColumn_combobox.selectRow($control, $row, validationName, $valuefield, $btnvalidate, $validationbrowse, controller);
                        return;
                    }
                } else {
                    if (e.which === 40) { // Down Arrow
        ...
        .on('click', '.btnvalidate', function() {
            try {
                if ((typeof $control.data('dropdown') != 'undefined') && ($control.data('dropdown') !== null)) {
                    FwBrowseColumn_combobox.closeDropDown($control, true);
                }
                else if ((typeof $control.attr('data-enabled') !== 'string') || ($control.attr('data-enabled') !== 'false')) {
                    FwBrowseColumn_combobox.validate($control, validationName, $valuefield, $btnvalidate, $validationbrowse, false);
                }
            } catch (ex) {
                FwFunc.showError(ex);
            }
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.databindcallback = function($control, $browse, dt, validationName, $valuefield, $btnvalidate, $validationbrowse, controller) {
    ...
    if (dt.Rows.length > 0) {
        ...
        $dropdown.on('mouseover', '.row', function() {
            var $row = jQuery(this);
            FwBrowseColumn_combobox.highlightRow($control, $row);
        });
        ...
        if ($control.data('dropdown') !== null) {
            FwBrowseColumn_combobox.closeDropDown($control, false);
        ...
        if (FwFunc.isDesktop()) {
            FwBrowseColumn_combobox.highlightRow($control, FwBrowseColumn_combobox.getHighlightedRow($control));
        }
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.closeDropDown = function($control, showAnimation) {
    if ((typeof $control.data('dropdown') !== 'undefined') && ($control.data('dropdown') !== null)) {
        if (showAnimation) {
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.validate = function($control, validationName, $valuefield, $btnvalidate, $validationbrowse, useSearchFieldValue) {
    var $validationSearchbox;

    FwBrowseColumn_combobox.clearSearchCriteria($validationbrowse);
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.selectRow = function($control, $row, validationName, $valuefield, $btnvalidate, $validationbrowse, controller) {
    var uniqueid, text;
    
    FwBrowseColumn_combobox.highlightRow($row);
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.clearSearchCriteria = function($validationbrowse) {
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.getHighlightedRow = function($control) {
    var $row = null;
    if ($control.data('dropdown') !== null) {
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.highlightRow = function($control, $row) {
...
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.highlightNextRow = function($control) {
    var $row = FwBrowseColumn_combobox.getHighlightedRow($control);
    if ($row !== null) {
        var $nextrow = $row.next('.row');
        if ($nextrow.length > 0) {
            FwBrowseColumn_combobox.highlightRow($control, $nextrow);
        }
    }
};
//---------------------------------------------------------------------------------
FwBrowseColumn_combobox.highlightPrevRow = function($control) {
    var $row = FwBrowseColumn_combobox.getHighlightedRow($control);
    if ($row !== null) {
        var $prevrow = $row.prev('.row');
        if ($prevrow.length > 0) {
            FwBrowseColumn_combobox.highlightRow($control, $prevrow);
        }
    }
};
//---------------------------------------------------------------------------------
