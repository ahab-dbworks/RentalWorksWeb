FwFormField_date = {};
//---------------------------------------------------------------------------------
FwFormField_date.renderDesignerHtml = function($control, html) {
    html.push(FwControl.generateDesignerHandle($control.attr('data-type'), $control.attr('id')));
    html.push('<div class="fwformfield-caption">' + $control.attr('data-caption') + '</div>');
    html.push('<div class="fwformfield-control">');
    html.push('<input class="fwformfield-value" type="text"');
    if ($control.attr('data-enabled') === 'false') {
        html.push(' disabled="disabled"');
    }
    html.push(' />');
    html.push('</div>');
    $control.html(html.join(''));
};
//---------------------------------------------------------------------------------
FwFormField_date.renderRuntimeHtml = function($control, html) {
    var isdesktop = jQuery('html').hasClass('desktop');
    var ismobile  = jQuery('html').hasClass('mobile');
    html.push('<div class="fwformfield-caption">' + $control.attr('data-caption') + '</div>');
    html.push('<div class="fwformfield-control">');
        html.push('<input class="fwformfield-value" type="text" autocapitalize="none"');
        if ($control.attr('data-enabled') === 'false') {
            html.push(' disabled="disabled"');
        }
        html.push(' />');
        html.push('<div class="add-on">');
            if (isdesktop) {
                html.push('<i class="material-icons">&#xE878;</i>'); //event
            } else if (ismobile) {
                html.push('<i class="material-icons md-dark btnvalidate">&#xE878;</i>'); //event
            }
        html.push('</div>');
        if ($control.attr('data-datenav') == 'true') {
            html.push('<div class="date-nav">');
                html.push('<div class="today">Today</div>');
                html.push('<div class="yesterday"><div class="icon-westarrow"></div></div>');
                html.push('<div class="tomorrow" ><div class="icon-eastarrow"></div></div>');
            html.push('</div>');
        }
    html.push('</div>');
    $control.html(html.join(''));

    $control.find('.fwformfield-value').inputmask('mm/dd/yyyy');
    $control.find('.fwformfield-value').datepicker({
        endDate:        (($control.attr('data-nofuture') == 'true') ? '+0d' : Infinity),
        autoclose:      true,
        format:         "mm/dd/yyyy",
        todayHighlight: true,
        todayBtn:       'linked'
    }).off('focus'); //MY 1/5/2015: Suppresses the date picker from opening on focus.

    $control
        .on('change', '.fwformfield-value', function() {
            try {
                if ($control.attr('data-nofuture') == 'true') {
                    var datevalue, today;
                    datevalue = new Date(jQuery(this).val());
                    today = new Date();
                    if (datevalue > today) {
                        jQuery(this).val('');
                        $control.addClass('error');
                        FwNotification.renderNotification('ERROR', 'Cannot select future dates.');
                    } else {
                        $control.removeClass('error');
                        jQuery(this).datepicker('update');
                    }
                } else {
                    jQuery(this).datepicker('update');
                }
            } catch(ex) {
                FwFunc.showError(ex);
            }
        })
        .on('click', '.add-on', function() {
            try {
                if ($control.attr('data-enabled') === 'true') {
                    $control.find('.fwformfield-value').datepicker('show');
                }
            } catch(ex) {
                FwFunc.showError(ex);
            }
        })
        .on('click', '.today', function() {
            var today;
            try {
                today = FwFunc.getDate();
                $control.find('.fwformfield-value').val(today).change();
            } catch(ex) {
                FwFunc.showError(ex);
            }
        })
        .on('click', '.yesterday', function() {
            var date, value;
            try {
                value = $control.find('.fwformfield-value').val();
                date = FwFunc.getDate(value, -1);
                $control.find('.fwformfield-value').val(date).change();
            } catch(ex) {
                FwFunc.showError(ex);
            }
        })
        .on('click', '.tomorrow', function() {
            var date, value;
            try {
                value = $control.find('.fwformfield-value').val();
                date = FwFunc.getDate(value, 1);
                $control.find('.fwformfield-value').val(date).change();
            } catch(ex) {
                FwFunc.showError(ex);
            }
        })
    ;
};
//---------------------------------------------------------------------------------
FwFormField_date.loadItems = function($control, items, hideEmptyItem) {

};
//---------------------------------------------------------------------------------
FwFormField_date.loadForm = function($fwformfield, table, field, value, text) {
    $fwformfield
        .attr('data-originalvalue', value)
        .find('.fwformfield-value')
            .val(value);
    $fwformfield.find('.fwformfield-value').datepicker('update');
};
//---------------------------------------------------------------------------------
FwFormField_date.disable = function($control) {

};
//---------------------------------------------------------------------------------
FwFormField_date.enable = function($control) {

};
//---------------------------------------------------------------------------------
FwFormField_date.getValue2 = function($fwformfield) {
    var value = $fwformfield.find('.fwformfield-value').val();
    return value;
};
//---------------------------------------------------------------------------------
FwFormField_date.setValue = function($fwformfield, value, text, firechangeevent) {
    var $inputvalue = $fwformfield.find('.fwformfield-value');
    $inputvalue.val(value); 
    $inputvalue.datepicker('update');
    if (firechangeevent) $inputvalue.change();
};
//---------------------------------------------------------------------------------
