...
FwRibbon.getDesignerProperties = function(data_type) {
    ...
    propClass = {
      ...
      , datatype: 'string'
      , attribute: 'class'
      , defaultvalue: 'fwcontrol fwribbon'
      , visible: false
      , enabled: false
    };
    // data-control
...
FwRibbon.renderDesignerHtml = function($control) {
    ...
    FwRibbon.setActiveTab($control, $activetab);
};
//---------------------------------------------------------------------------------
...
FwRibbon.generateSplitDropDownModuleBtn = function(securityid, caption, modulenav, imgurl, subitems) {
    ...
    for (var i = 0; i < subitems.length; i++) {
        $modulebtn.on('click', '#ddBtn' + subitems[i].id, function() {
            ...
            subitem = jQuery.grep(subitems, function(e) { return e.id == id });
