...
FwCharge.renderRuntimeHtml = function($control) {
    var html, data_rendermode, appchargetemplates, activeTemplate, data_datafield, width, maxlength;
    ...
    for (var i = 0; i < activeTemplate.fields.length; i++) {
        if (activeTemplate.fields[i].primary == 'T') {
            ...
            width     = (parseInt(maxlength) + 1) * ((parseInt(maxlength) < 10) ? 16 : 12);
            html.push('<div data-control="FwFormField" data-type="text" class="fwcontrol fwformfield" data-caption="' + activeTemplate.fields[i].caption + '" data-datafield="' + data_datafield + (i+1) + '" data-maxlength="' + maxlength + '" style="float:left;width:' + width + 'px;"></div>');
        }
    }
...
FwCharge.rerenderRuntimeHtml = function($form, tables) {
    var html, appchargetemplates, activeTemplate, data_datafield, $fwchargefields, width, maxlength;
