...
FwBrowse.databindcallback = function($control, dt) {
    ...
    try {
        ...
        if ($control.attr('data-type') === 'Grid') {
            ...
            $trs.on('click', '.editablefield', function (e) {
                try {
                    var $td = jQuery(this);
...
//---------------------------------------------------------------------------------
FwBrowse.getController = function ($browse) {
    var controllername;
    var controller; // default value of controller will be undefined if not found
    if (typeof $browse.attr('data-name') === 'string' && $browse.attr('data-name').length > 0) {
        controllername = module + 'Controller';
    }
    else if (typeof $browse.attr('data-controller') === 'string' && $browse.attr('data-controller').length > 0) {
