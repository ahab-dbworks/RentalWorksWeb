...
FwValidation.init = function($control) {
    ...
    $validationbrowse.data('ondatabind', function (request) {
        ...
        else if ((typeof $object.attr('data-name') === 'string') && (typeof window[$object.attr('data-name') + 'Controller'] === 'object') && (typeof window[$object.attr('data-name') + 'Controller']['beforeValidate'] === 'function')) {
            if ($object.attr('data-type') === 'Grid') {
    ...
    focusValidationSearchBox = function ($validationbrowse) {
        setTimeout(function () {
            var $searchBox = $validationbrowse.find('thead td[data-visible="true"] .search input')[0];
            $searchBox.focus();
        }, 400);
    };

    closeValidationPopup = function ($validationbrowse) {
...
FwValidation.selectRow = function($tr, validationName, $valuefield, $searchfield, $btnvalidate, $validationbrowse, controller) {
    ...
    valuedatafield = $valuefield.closest('.fwformfield').attr('data-datafield');
    if ((typeof controller === 'string') && (typeof window[controller] === 'object') && (typeof window[controller]['loadRelatedValidationFields'] === 'function')) {
        window[controller]['loadRelatedValidationFields'](validationName, $valuefield, $tr, valuedatafield);
    }
    FwValidation.clearSearchCriteria($validationbrowse, $btnvalidate);
