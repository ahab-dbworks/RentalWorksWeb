...
FwApplication.prototype.debugCss = function (idlength) {
    ...
    formfields = ['text', 'number', 'password', 'select', 'date', 'time', 'email', 'url', 'phone', 'validation', 'money', 'zipcode', 'percent', 'ssn', 'color', 'checkbox', 'radio', 'searchbox', 'textarea'];

    for (var i = 0; i < formfields.length; i++) {
        var field, fieldname;
        field     = formfields[i];
        fieldname = field.toLowerCase().replace(/\b[a-z]/g, function(letter) {return letter.toUpperCase();});
        html.push('<div class="fwcontrol fwcontainer fwform-section" data-control="FwContainer" data-type="section" data-caption="' + fieldname + ' Section" style="float:left;width:200px;">');
    ...
    FwControl.renderRuntimeControls($appendContent.find('.fwcontrol').addBack());
