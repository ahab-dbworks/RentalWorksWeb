...
namespace Fw.MSBuildTasks
{
    public class FwJSAppBuilder : Task
    {
                    ...
                    for (int i = 0; i < config.Targets.Count; i++)
                    {
                        if (this.Publish == config.Targets[i].Publish)
                        {
                            ...
                            if (buildpages)
                            {
                                ...
                                // Fw Controls
                                pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls");
                                ...
                                // Fw Pages
                                pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Pages");
                                if (Directory.Exists(pathModules))
                                {
                                    pathModuleArray = Directory.GetFiles(pathModules, "*.htm", SearchOption.AllDirectories);
                                    foreach (string pathModule in pathModuleArray)
                            ...
                            if (buildmodules)
                            {
                                // Fw Modules
                                sbModules = new StringBuilder();
                                pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Modules");
                                if (Directory.Exists(pathModules))
                                {
                                ...
                                // Fw SubModules
                                pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\SubModules");
                                if (Directory.Exists(pathModules))
                                {
                                    pathModuleList = getModuleDirectories(pathModules);
                                    foreach (string pathModule in pathModuleList)
                            ...
                            sbModules = new StringBuilder();
                            // Fw Grids
                            pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Grids");
                            if (Directory.Exists(pathModules))
                            {
                                pathModuleList = getModuleDirectories(pathModules);
                                foreach (string pathModule in pathModuleList)
                            ...
                            sbModules = new StringBuilder();
                            // Fw Validations
                            pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Validations");
                            if (Directory.Exists(pathModules))
                            {
                                pathModuleList = getModuleDirectories(pathModules);
                                foreach (string pathModule in pathModuleList)
                            ...
                            if (buildreports)
                            {
                                sbModules = new StringBuilder();
                                // Fw Reports
                                pathModules  = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Reports");
                                if (Directory.Exists(pathModules))
                                {
                                    pathModuleList = getModuleDirectories(pathModules);
                                    foreach (string pathModule in pathModuleList)
                            ...
                            foreach (SourceFile sourceFile in config.SourceFiles)
                            {
                                if (sourceFile.ReplaceField == "{{{javascripts}}}")
                                {
                                    ...
                                    string fwpath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content");

                                    jsFiles = new List<string>();

                                    jspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source");
                                ...
                                if (sourceFile.ReplaceField == "{{{stylesheets}}}")
                                {
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls");
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\Grids");
                                    if (Directory.Exists(csspath))
                                    {
                                        newInputFiles = Directory.GetFiles(csspath, "*.css", SearchOption.AllDirectories);
                                        cssFiles.AddRange(newInputFiles);
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\Modules");
                                    if (Directory.Exists(csspath))
                                    {
                                        newInputFiles = Directory.GetFiles(csspath, "*.css", SearchOption.AllDirectories);
                                        cssFiles.AddRange(newInputFiles);
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\Pages");
                                    if (Directory.Exists(csspath))
                                    {
                                        newInputFiles = Directory.GetFiles(csspath, "*.css", SearchOption.AllDirectories);
                                        cssFiles.AddRange(newInputFiles);
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\Reports");
                                    if (Directory.Exists(csspath))
                                    {
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\SubModules");
                                    if (Directory.Exists(csspath))
                                    {
                                        newInputFiles = Directory.GetFiles(csspath, "*.css", SearchOption.AllDirectories);
                                        cssFiles.AddRange(newInputFiles);
                                    ...
                                    csspath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\content\source\Controls\Modules\Validations");
                                    if (Directory.Exists(csspath))
                                    {
                                        newInputFiles = Directory.GetFiles(csspath, "*.css", SearchOption.AllDirectories);
                                        cssFiles.AddRange(newInputFiles);
                                ...
                                else  // Publish is true
                                {
                                    ...
                                    if ((config.Targets[i].Minify) && (config.SourceFiles[j].Minify))
                                    {
                                        ...
                                        if (Path.GetExtension(sourceFiles_minifiedFile) == ".js")
                                        {
                                            ...
                                            //commandLine.AppendSwitchIfNotNull("-jar ", Path.Combine(fwPath, @"Build\Fw.MsBuild\yuicompressor-2.4.7.jar"));
                                            ...
                                            commandLine.AppendSwitchIfNotNull("-jar ", Path.Combine(solutionDir, @"Fw\Build\Fw.MsBuild\closure-compiler\compiler.jar"));
                                        ...
                                        {
                                            commandLine = new CommandLineBuilder();
                                            commandLine.AppendSwitchIfNotNull("-jar ", Path.Combine(solutionDir, @"Fw\Build\Fw.MsBuild\yuicompressor-2.4.7.jar"));
                                            commandLine.AppendSwitchIfNotNull("-o ", sourceFiles_minifiedFile);
                                            commandLine.AppendFileNameIfNotNull(sourceFiles_combinedOutputPath);
                                            javaPath = GetJavaInstallationPath();
                                            if (string.IsNullOrEmpty(javaPath))
        ...
        private void BuildFwReferencesJsFile(string solutionDir)
        {
            ...
            string fwjsoncontentpath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\Content\");
            if (Directory.Exists(fwjsoncontentpath))
            {
                ...
                string[] fwexcludescripts = File.ReadAllLines(Path.Combine(solutionDir, @"Fw\src\Fw.Json\scripts\_referencesToExclude.txt"));
                ...
                string fwreferencespath = Path.Combine(solutionDir, @"Fw\src\Fw.Json\scripts\_references.js");
