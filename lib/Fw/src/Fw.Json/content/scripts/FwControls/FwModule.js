...
FwModule.openSubModuleTab = function($browse, $form) {
    $browse.hide();
    $browse.after($form);
    jQuery('#' + $browse.parent().attr('data-tabid')).find('.form-caption').html('New ' + $form.attr('data-caption'));
    jQuery('#' + $browse.parent().attr('data-tabid')).find('.submodule-caption').show();
};
//----------------------------------------------------------------------------------------------
FwModule.openFormTab = function($form, $object, tabname, tabhasclose, tabtype, setactive) {
    var $formtabcontrol, tabids;
...
FwModule.addBrowseMenu = function($browse) {
    ...
    if ((typeof $browse.attr('data-hasinactive') === 'string') && ($browse.attr('data-hasinactive') === 'true') && (typeof $browse.attr('data-activeinactiveview') === 'string')) {
...
FwModule.loadForm = function(module, $form) {
    ...
    FwServices.module.method(request, module, 'Load', $form,
        ...
        function(response) {
            ...
            try {
                if (!FwAppData.useWebApi && typeof response.tables !== 'object') {
...
FwModule.saveForm = function(module, $form, closetab, navigationpath) {
    ...
    if (isValid) {
        ...
        FwServices.module.method(request, module, 'Save', $form, function(response) {
            var $formfields, controller, issubmodule, $browse;
            ...
            if (FwAppData.useWebApi) {
                if (!closetab) {
                    ...
                    $browse = jQuery('.fwbrowse[data-controller="' + controller + '"]')
                    if ($browse.length > 0) {
                    ...
                    if (typeof window[controller]['afterSave'] === 'function') {
                        window[controller]['afterSave']($form);
                    }
                } else if (closetab) {
...
FwModule.checkDuplicate = function($form, $fieldtocheck) {
    ...
    if (typeof window[controller].Module !== 'undefined') {
        ...
        runcheck = runcheck && !FwAppData.useWebApi;
        if (runcheck) {
            FwServices.module.method(request, window[controller].Module, 'ValidateDuplicate', $form,
                // onSuccess
                function(response) {
                    try {
                        if ((!FwAppData.useWebApi && response.duplicate == true) || (FwappData.useWebApi && response)) {
