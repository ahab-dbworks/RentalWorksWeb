...
//---------------------------------------------------------------------------------
FwBrowseColumn_appdocumentversion.databindfield = function($browse, $field, dt, dtRow, field, $tr) {
...
FwBrowseColumn_appdocumentversion.setFieldViewMode = function($browse, $field, $tr, html) {
    ...
    if (version !== '0') {
        ...
        html.push('  <div><img class="btnShowVersions" src="' + applicationConfig.appbaseurl + 'theme/fwimages/icons/128/browsesearch.001.png" style="width:16px;height:16px;vertical-align:middle;cursor:pointer;" />' + version + '</div>');
        html.push('</div>');
    } else {
    ...
    if (html.length > 0) {
        $control.on('click', '.btnShowVersions', function(event) {
            ...
            try {
                ...
                $popupbrowse = jQuery('<div>').append(jQuery('#tmpl-grids-AppDocumentVersionBrowse').html());
