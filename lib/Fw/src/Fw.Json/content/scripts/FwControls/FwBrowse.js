                ...
                switch(code) {
                    case 13: //Enter Key
                        ...
                        if (jQuery.inArray(e.target.offsetParent, $control.find('thead tr td .field div.search')) > -1) {
                            ...
                            var $tr, browseuniqueids, formuniqueids, $form, issubmodule;

                            $tr             = $control.find('tbody tr.selected');
                            formuniqueids   = FwBrowse.getRowFormUniqueIds($control, $tr);
                            browseuniqueids = FwBrowse.getRowBrowseUniqueIds($control, $tr);
                            ...
                            if ((typeof $form === 'undefined') || ($form.length == 0)) {
                                ...
                                //FwModule.loadForm(window[$form.attr('data-controller')].Module, $form);
                                jQuery('#' + tabid).click();
                            }
                    ...
                    case 32: //Space Bar Key
                        ...
                        e.preventDefault;
                        e.stopPropagation();
                        $tr = $control.find('tbody tr:focus');
...
FwBrowse.renderRuntimeHtml = function($control) {
    ...
    switch(data_rendermode) {
        ...
        case 'template':
            ...
            if ($control.attr('data-type') === 'Grid') {
                html.push('<div class="browsecaption">' + $control.attr('data-caption') + '</div>');
            }
                ...
                $theadfields.each(function(index, element) {
                    ...
                    html.push($field[0].outerHTML.replace('</div>',''));
                        html.push('<div class="fieldcaption" style="min-width:' + width + ';">');
                            ...
                            html.push('<div class="sort"><div class="sort-icon"></div></div>');
            ...
            function getcolumnoptionbutton(text, imageurl) {
                var  html;
                html = [];
                html.push('<div class="columnoptions-button">');
                    html.push('<div class="columnoptions-button-icon">' + ((imageurl != '') ? '<img src="' + imageurl + '" class="icon">' : '') + '</div>');
            ...
            $theadfields.each(function(index, element) {
                ...
                if (showsort) {
                    ...
                    $sortascbtn = getcolumnoptionbutton('Sort Ascending', applicationConfig.appbaseurl + applicationConfig.appvirtualdirectory + 'theme/fwimages/icons/128/sortasc_dark.png');
                    $sortascbtn.on('click', function() {
                        $theadfield.attr('data-sort', 'asc');
                        FwBrowse.databind($control);
                    });
                    $columnoptions.append($sortascbtn);

                    $sortdescbtn = getcolumnoptionbutton('Sort Descending', applicationConfig.appbaseurl + applicationConfig.appvirtualdirectory + 'theme/fwimages/icons/128/sortdesc_dark.png');
                    $sortdescbtn.on('click', function() {
                        $theadfield.attr('data-sort', 'desc');
                        FwBrowse.databind($control);
                    });
                    $columnoptions.append($sortdescbtn);

                    $sortoffbtn = getcolumnoptionbutton('Sort Off', applicationConfig.appbaseurl + applicationConfig.appvirtualdirectory + 'theme/fwimages/icons/128/sortoff_dark.png');
                    $sortoffbtn.on('click', function() {
                        $theadfield.attr('data-sort', 'off');
                        FwBrowse.databind($control);
                    });
            ...
            if ($control.attr('data-type') === 'Grid') {
                var $menu = FwMenu.getMenuControl('grid');
    ...
    if ((typeof $control.attr('data-controller') !== 'undefined') && ($control.attr('data-controller') !== '')) {
        ...
        //if (gridNode === null) {
        //    throw 'Application tree is missing a Grid node with the controller: ' + controller;
        //}
        if (gridNode !== null) {
            ...
            if (gridMenuBar !== null) {
                if (gridMenuBar.properties.visible === 'T') {
                    ...
                    if ((gridSubMenu !== null) && (gridSubMenu.properties.visible === 'T')) {
                        var $submenubtn    = FwMenu.addSubMenu($menu);
                        for (var gridSubMenuGroupIndex = 0; gridSubMenuGroupIndex < gridSubMenu.children.length; gridSubMenuGroupIndex++) {
                            ...
                            if ((gridSubMenuGroup && gridSubMenuGroup.properties.nodetype === 'SubMenuGroup') && (gridSubMenuGroup.properties.visible === 'T')) {
                                var $submenucolumn = FwMenu.addSubMenuColumn($submenubtn);
                                var $optiongroup   = FwMenu.addSubMenuGroup($submenucolumn, gridSubMenuGroup.properties.caption, gridSubMenuGroup.id);
                                for (var gridSubMenuItemIndex = 0; gridSubMenuItemIndex < gridSubMenuGroup.children.length; gridSubMenuItemIndex++) {        
                                    ...
                                    if ((gridSubMenuItem && gridSubMenuItem.properties.nodetype === 'SubMenuItem') && (gridSubMenuItem.properties.visible === 'T')) {
                                        var $submenuitem = FwMenu.addSubMenuBtn($optiongroup, gridSubMenuItem.properties.caption, gridSubMenuItem.id);
                    ...
                    if (hasNew) {
                        var $new = FwMenu.addStandardBtn($menu, FwLanguages.translate('New'), gridNewMenuBarButton.id);
                    ...
                    if (hasEdit) {
                        var $edit = FwMenu.addStandardBtn($menu, FwLanguages.translate('Edit'), gridEditMenuBarButton.id);
                    ...
                    if (hasDelete) {
                        var $delete = FwMenu.addStandardBtn($menu, FwLanguages.translate('Delete'), gridDeleteMenuBarButton.id);
                    ...
                    if (hasSave) {
                        var $save   = FwMenu.addStandardBtn($menu, FwLanguages.translate('Save'));
                    ...
                    if (hasCancel) {
                        var $cancel = FwMenu.addStandardBtn($menu, 'Cancel');
                        $cancel.attr('data-type', 'CancelButton');
                        $cancel.css('display', 'none').on('click', function() {
                            var $inputFile;
                            try {
                                $inputFile = $control.find('tr.editrow input[type="file"]');
...
FwBrowse.getRequest = function($control) {
    var request, $fields, orderby, $field, $txtSearch, browsedatafield, value, sort, module;
    orderby = [];
    request = {
        module:               '',
        searchfields:         [],
    ...
    if (typeof $control.attr('data-activeinactiveview') === 'string') {
        switch ($control.attr('data-activeinactiveview')) {
            case 'all':
                break;
            case 'active':
            ...
            if ($control.attr('data-type') === 'Grid') {
                ...
                FwServices.module.method(request, request.module, 'Browse', $control, function(response) {
                    try {
                        ...
                        if (FwAppData.useWebApi && typeof controller.apiurl !== 'undefined')
                        {
                            FwBrowse.databindcallback($control, response);
                        } else {
                            FwBrowse.databindcallback($control, response.browse);
...
FwBrowse.databindcallback = function($control, dt) {
    ...
    try {
        ...
        for (rowIndex = 0; rowIndex < dt.Rows.length; rowIndex++) {
            ...
            for (columnIndex = 0; columnIndex < columns.length; columnIndex++) {
                ...
                for (var fieldIndex = 0; fieldIndex < scrollerCol.fields.length; fieldIndex++) {
                    ...
                    if (dtCellValue != '') { 
                        $field.attr('data-originalvalue', dtCellValue);
                    }
            ...
            case 'Browse':
                htmlPager.push('<div class="col1" style="width:33%;overflow:hidden;float:left;">');
                    htmlPager.push('<div class="btnRefresh" title="Refresh" tabindex="0">');
                        htmlPager.push('<div class="refreshicon"></div>');
                    htmlPager.push('</div>');
        ...
        setTimeout(function() {
            ...
            if (rowcount > FwBrowse.getSelectedIndex($control)) {
...
FwBrowse.setFieldViewMode = function($control, $field, $tr) {
    var browsedatatype, originalvalue, originaltext, appdocumentid, appimageid, filename, fileextension, $browse, $form, controller, $option, uniqueids, html;
    browsedatatype = (typeof $field.attr('data-browsedatatype') === 'string') ? $field.attr('data-browsedatatype') : '';
    ...
    fileextension  = (typeof $field.attr('data-fileextension')  === 'string') ? $field.attr('data-fileextension').toLowerCase() : '';
...
FwBrowse.saveRow = function($control, $tr) {
    ...
    mode = $tr.hasClass('newmode') ? 'NEW' : $tr.hasClass('editmode') ? 'EDIT' : '';
    ...
    if (isvalid) {
        ...
        FwServices.grid.method(request, name, 'Save', $control, function(response) {
            FwBrowse.search($control);
            $control.attr('data-mode', 'VIEW');
            if (($control.attr('data-type') == 'Grid') && (typeof $control.attr('data-controller') !== 'undefined') && ($control.attr('data-controller') !== '')) {
                var controller;
