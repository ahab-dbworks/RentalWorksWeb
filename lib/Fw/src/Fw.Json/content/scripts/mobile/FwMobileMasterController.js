...
FwMobileMasterController.getMasterView = function(viewModel, properties) {
    ...
    FwMobileMasterController.tabcontrols
        .on('click', '.tab', function() {
            var $this, $tabs;
            $this = jQuery(this);
            $tabs = FwMobileMasterController.tabcontrols.find('.tab');
    ...
    html.push('<div class="menu">');
        html.push('<div class="menu-body">');
            html.push('<div class="menu-body-top">');
                html.push('<div class="apptitle center" style="padding-top:10px 0;background-color:rgba(0,0,0,.4);font-size:20px;">' + application.htmlname + '</div>');
            ...
            html.push('<div class="menu-body-footer center" style="font-size:10px;padding:5px 0;background-color:rgba(0,0,0,.4);width:100%;">' + application.name + ' v' + applicationConfig.version + '</div>');
        html.push('</div>');
...
FwMobileMasterController.generateMenuLinks = function($menu) {
        ...
        if (nodeModule.properties.visible === 'T') {
            switch (nodeModule.properties.nodetype) {
                case 'Module':
                    ...
                    if (hasusertype) {
                        $link = FwMobileMasterController.generateLink($menu, nodeModule.properties.caption, nodeModule.properties.iconurl, nodeModule.properties.modulenav); $menu.find('.menu-body-links').append($link);
                    }
                    break;
            }
        }
    ...
    $link      = FwMobileMasterController.generateLink($menu, 'Logoff',      'theme/images/icons/128/logoff.png',      'account/logoff');      $menu.find('.menu-body-links').append($link);
};
//----------------------------------------------------------------------------------------------
