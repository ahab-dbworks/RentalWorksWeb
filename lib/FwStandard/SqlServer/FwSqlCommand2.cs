...
namespace FwStandard.SqlServer
{
    ...
    public class FwSqlCommand2 : IDisposable
    {
        //------------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------------------
        public FwSqlCommand2(FwSqlConnection conn, int timeout)
        {
        ...
        //------------------------------------------------------------------------------------
        public FwSqlCommand2(FwSqlConnection conn, string storedProcedureName, int timeout)
        {
            this.sqlConnection = conn;
            this.sqlCommand = new SqlCommand();
            this.sqlCommand.Connection = this.sqlConnection.GetConnection();
        ...
        public static FwDatabaseField GetData(FwSqlConnection conn, int timeout, string tablename, string wherecolumn, string wherecolumnvalue, string selectcolumn)
        {
            ...
            qry = new FwSqlCommand2(conn, timeout);
        ...
        public static string GetStringData(FwSqlConnection conn, int timeout, string tablename, string wherecolumn, string wherecolumnvalue, string selectcolumn)
        {
            ...
            field = FwSqlCommand2.GetData(conn, timeout, tablename, wherecolumn, wherecolumnvalue, selectcolumn);
        ...
        public static int SetData(FwSqlConnection conn, int timeout, string table, string wherecolumn, string wherecolumnvalue, string updatecolumn, string updatecolumnvalue)
        {
            ...
            cmd = new FwSqlCommand2(conn, timeout);
        ...
        //------------------------------------------------------------------------------------
        public int Insert(bool openAndCloseConnection, string tablename, InsertRequestDto request)
        {
            StringBuilder insertColumnsNames, insertParameterNames;
            ...
            try
            {
                ...
                insertParameterNames = new StringBuilder();
                var i = 0;
                foreach (KeyValuePair<string, object> uniqueId in request.uniqueIds)
                {
                    if (i > 0)
                    {
                        insertColumnsNames.Append(",");
                    ...
                    insertColumnsNames.Append("[");
                    insertColumnsNames.Append(uniqueId.Key);
                    insertColumnsNames.Append("]");
                    insertParameterNames.Append("@" + uniqueId.Key);
                    this.AddParameter("@" + uniqueId.Key, uniqueId.Value);
                    i++;
                }
                foreach (KeyValuePair<string, object> field in request.fields)
                {
                    if (i > 0)
                    {
                        insertColumnsNames.Append(",");
                    ...
                    insertColumnsNames.Append("[");
                    insertColumnsNames.Append(field.Key);
                    insertColumnsNames.Append("]");
                    insertParameterNames.Append("@" + field.Key);
                    this.AddParameter("@" + field.Key, field.Value);
                    i++;
                }
                this.sqlCommand.CommandText = "insert into " + tablename + "(" + insertColumnsNames + ")\nvalues (" + insertParameterNames + ")";
                //FwFunc.WriteLog("Query:\n" + sqlCommand.CommandText);
        ...
        //------------------------------------------------------------------------------------
        public int Update(bool openAndCloseConnection, string tablename, UpdateRequestDto request)
        {
            try
            {
                //FwFunc.WriteLog("Begin FwSqlCommand:ExecuteInsertQuery()");
                ...
                foreach (KeyValuePair<string, object> field in request.fields)
                {
                    ...
                    sql.AppendLine(prefix + field.Key + " = " + field.Value);
                }
                i = 0;
                foreach (KeyValuePair<string, object> uniqueId in request.uniqueIds)
                {
                    ...
                    sql.AppendLine("  " + uniqueId.Key + " = " + uniqueId.Value);
                    i++;
                }
