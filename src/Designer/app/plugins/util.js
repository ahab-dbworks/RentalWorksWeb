...
const Util = (() => {
  ...
  const Util = {
    ...
    getSelectorFromElement(element) {
      ...
      try {
        const jQueryselector = jQuery(document).find(selector)
      ...
      for (const property in configTypes) {
        if (Object.prototype.hasOwnProperty.call(configTypes, property)) {
          ...
          if (!new RegExp(expectedTypes).test(valueType)) {
            throw new Error(
              `jQuery{componentName.toUpperCase()}: ` +
              `Option "jQuery{property}" provided type "jQuery{valueType}" ` +
              `but expected type "jQuery{expectedTypes}".`)
          }
        }
      }
    }
