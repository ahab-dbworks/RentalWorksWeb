...
FwTabs.init = function($control) {
    ...
    $control
        ...
        .off('click', '> .runtime > .tabs > .tab')
        .on('click', '> .runtime > .tabs > .tab', function() {
            var $tab, $tabs;
            try {
                $tab  = jQuery(this);
        ...
        .off('click', '> .runtime > .tabs > .tab .delete')
        .on('click', '> .runtime > .tabs > .tab .delete', function(event) {
...
//---------------------------------------------------------------------------------
FwTabs.setActiveTab = function($control, $tab) {
    var $tabpage, tabid, tabpageid, allowTabChange, $tabs;
    allowTabChange = true;
...
//---------------------------------------------------------------------------------
FwTabs.getHtmlTag = function(data_type) {
...
//---------------------------------------------------------------------------------
FwTabs.getDesignerProperties = function(data_type) {
    var properties = [], propId, propClass, propDataControl, propDataType, propRenderMode, propDataCaption, propDataEnabled, propDataOriginalValue, propDataImageUrl, propDataField, propDataFieldCount, propFieldCount;
    //id
    propId = {
        caption: 'ID'
      , datatype: 'string'
    ...
    propId.defaultvalue = FwControl.generateControlId('tabs');
    //class
    propClass = {
        caption: 'CSS Class'
      , datatype: 'string'
      , attribute: 'class'
      , defaultvalue: 'fwcontrol fwtabs'
      , visible: false
      , enabled: false
    };
    // data-control
...
//---------------------------------------------------------------------------------
FwTabs.renderDesignerHtml = function($control) {
...
//---------------------------------------------------------------------------------
FwTabs.renderRuntimeHtml = function($control) {
    var data_type, data_rendermode, html, $tabsChildren, $tabpagesChildren, $tabs, $newtab, $activetab;
    data_type = $control.attr('data-type');
    data_rendermode = $control.attr('data-rendermode');
    ...
    $tabsChildren
        .each(function(index, tab) {
            ...
            if ($tab.hasClass('submodule')) {
                tabHtml.push('<div class="submodule-caption" style="display:none;">');
                    tabHtml.push('<div style="margin:0 3px;float:left;">|</div>');
                    tabHtml.push('<div class="form-caption"></div>');
...
//---------------------------------------------------------------------------------
FwTabs.renderTemplateHtml = function($control) {
...
//---------------------------------------------------------------------------------
FwTabs.addTab = function($control, caption, hasClose, tabType, setActive) {
    var $newtab, $tab, $tabpage, tabHtml, newtabids;
    newtabids = {};
    try {
        ...
        $control.children('.runtime').children('.tabs').append($tab);
        $control.children('.runtime').children('.tabpages').append($tabpage);
        if (setActive == true) {
            FwTabs.setActiveTab($control, $tab);
        }
    } catch (ex) {
...
//---------------------------------------------------------------------------------
FwTabs.removeTabBySelector = function($control, tabselector) {
    var $tab;
    $tab = $control.find(tabselector);
    FwTabs.removeTab($tab);
};
//---------------------------------------------------------------------------------
FwTabs.closeTabs = function($control) {
    var $tabs, $tabpages;
...
//---------------------------------------------------------------------------------
FwTabs.getTabPageByElement = function($element) {
    var $tabpage
    $tabpage = $element.closest('.tabpage');
    return $tabpage;
};
//---------------------------------------------------------------------------------
FwTabs.getTabByElement = function($element) {
...
//---------------------------------------------------------------------------------
FwTabs.getTabsByTab = function($tab) {
    var $elementtab, $tabs;
    $tabs = $tab.closest('.tabs').find('.tab');
    return $tabs;
};
//---------------------------------------------------------------------------------
FwTabs.getTabByCaption = function($element, tabcaption) {
...
//---------------------------------------------------------------------------------
FwTabs.getTabPageByTab = function($tab) {
...
//---------------------------------------------------------------------------------
FwTabs.disableTab = function($tab) {
    $tab.attr('data-enabled', 'false');
};
//---------------------------------------------------------------------------------
FwTabs.enableTab = function($tab) {
    $tab.removeAttr('data-enabled');
};
//---------------------------------------------------------------------------------
FwTabs.showTab = function($tab) {
    $tab.removeAttr('data-tabdisplay');
    $tab.removeAttr('data-enabled');
};
//---------------------------------------------------------------------------------
FwTabs.hideTab = function($tab) {
