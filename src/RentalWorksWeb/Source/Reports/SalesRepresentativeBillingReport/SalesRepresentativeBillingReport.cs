...
{
    class SalesRepresentativeBillingReport : RwReport
    {
        //---------------------------------------------------------------------------------------------
        protected override string getReportName() { return "Sales Representative Billing"; }
        //---------------------------------------------------------------------------------------------
        ...
        protected override string renderBodyHtml(string styletemplate, string bodytemplate, PrintOptions printOptions)
        {
            string html;
            dynamic statuslist;
            FwJsonDataTable dtSalesRepresentativeBillingReport;
            StringBuilder sb;

            statuslist = request.parameters.statuslist;
            dtSalesRepresentativeBillingReport = GetInvoiceSummaryReport(statuslist);
            ...
            sb.Replace("[TotalRows]", "Total Rows: " + dtSalesRepresentativeBillingReport.Rows.Count);
            html = sb.ToString();
            html = this.applyTableToTemplate(html, "details", dtSalesRepresentativeBillingReport);

            return html;
        }
        //---------------------------------------------------------------------------------------------
        protected FwJsonDataTable GetInvoiceSummaryReport(dynamic statuslist)                                        // needs changing
        {
            FwSqlSelect select;
            FwSqlCommand qry;
