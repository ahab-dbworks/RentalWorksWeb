//----------------------------------------------------------------------------------------------
var ContactCompanyGridController = {
    Module: 'ContactCompanyGrid'
};
//----------------------------------------------------------------------------------------------
ContactCompanyGridController.init = function ($control) {

};
//----------------------------------------------------------------------------------------------
ContactCompanyGridController.afterDelete = function($control, $tr) {
    var $form, $contactNoteBrowse;

    $form              = $control.closest('.fwform');
    $contactNoteBrowse = $form.find('div[data-name="ContactNoteGrid"][data-control="FwBrowse"]');
    FwBrowse.search($contactNoteBrowse);
}
//----------------------------------------------------------------------------------------------
ContactCompanyGridController.addGridSubMenu = function($control, $menu) {
    // Activate
                ...
                $yes.on('click', function() {
                    ...
                    FwBrowse.getGridData($control, request, function(response) {
                        try {
                            ...
                            FwBrowse.search($form.find('div[data-name="ContactNoteGrid"][data-control="FwBrowse"]'));
                        } catch(ex) {
                            FwFunc.showError(ex);
                        }
                    });
            ...
            if ($trselected.hasClass('editrow')) {
                ...
                $yes.on('click', function() {
                    request = {
                        method:        'SetCompanyContactStatus',
                        active:        'F',
                        compcontactid: $trselected.find('div[data-browsedatafield="CompanyContactId"]').attr('data-originalvalue'),
                        contactid:     $trselected.find('div[data-browsedatafield="ContactId"]').attr('data-originalvalue')
                    }
                    FwBrowse.getGridData($control, request, function(response) {
                        try {
