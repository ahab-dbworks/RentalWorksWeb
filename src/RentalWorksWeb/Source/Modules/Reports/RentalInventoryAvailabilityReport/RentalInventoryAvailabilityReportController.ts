...
class RentalInventoryAvailabilityReport extends FwWebApiReport {
    ...
    dateValidation = function ($form, event) {
        ...
        if ($element.attr('data-datafield') === 'FromDate' && parsedFromDate < todayParsed) {
            $form.find('div[data-datafield="FromDate"]').addClass('error dev-add-error');
            ...
            $form.find('div[data-datafield="ToDate"]').addClass('error dev-add-error');
            FwNotification.renderNotification('WARNING', "Your chosen 'To Date' is before 'From Date'.");
        }
        else {
            $form.find('div[data-datafield="FromDate"]').removeClass('error dev-add-error');
            $form.find('div[data-datafield="ToDate"]').removeClass('error dev-add-error');
        }
    }
    //----------------------------------------------------------------------------------------------
};
