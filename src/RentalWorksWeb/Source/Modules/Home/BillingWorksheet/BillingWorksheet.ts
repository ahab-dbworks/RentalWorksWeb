...
//-----------------------------------------------------------------------------------------------------
//Print BillingWorksheet menu item
FwApplicationTree.clickEvents[Constants.Modules.Home.BillingWorksheet.form.menuItems.PrintWorksheet.id] = function (e: JQuery.ClickEvent) {
    try {
        const $form = jQuery(this).closest('.fwform');
        $form.find('.print-worksheet').trigger('click');
    }
    catch (ex) {
        FwFunc.showError(ex);
    }
