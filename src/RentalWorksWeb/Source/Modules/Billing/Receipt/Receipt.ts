...
class Receipt {
    ...
    addBrowseMenuItems(options: IAddBrowseMenuOptions): void {
        ...
        // Reports Group
        const $reportCol = FwMenu.addSubMenuColumn(options.$menu);
        const $reportGroup = FwMenu.addSubMenuGroup($reportCol, 'Reports');
        FwMenu.addSubMenuItem($reportGroup, 'Print', 'PVylYE8XDyxP', (e) => {
            try {
                this.printReceiptInvoicesFromBrowse(options.$browse);
            } catch (ex) {
                FwFunc.showError(ex);
