    ...
    */
                        ...
                        if ($submenubtn !== null) {
                            ...
                            if (gridSubMenu !== null) {
                                for (let gridSubMenuGroupIndex = 0; gridSubMenuGroupIndex < gridSubMenu.children.length; gridSubMenuGroupIndex++) {
                                    const gridSubMenuGroup = gridSubMenu.children[gridSubMenuGroupIndex];
                                    if ((gridSubMenuGroup && gridSubMenuGroup.properties.nodetype === 'SubMenuGroup') && (gridSubMenuGroup.properties.visible === 'T')) {
                                        const $submenucolumn = FwGridMenu.addSubMenuColumn($submenubtn);
                                        const $optiongroup = FwGridMenu.addSubMenuGroup($submenucolumn, gridSubMenuGroup.properties.caption, gridSubMenuGroup.id);
    ...
    async databind($control): Promise<any> {
        ...
        FwAppData.apiMethod(true, 'GET', window[controller].apiurl + '/emptyobject', null, FwServices.defaultTimeout, function onSuccess(response) {
            let dateField = $control.find('.datequery');
            let textField = $control.find('.textquery');
            let booleanField = $control.find('.booleanquery');
            ...
            window['FwFormField_select'].loadItems($control.find('.datafieldselect'), findFields, false);
            window['FwFormField_select'].loadItems($control.find('.andor'), [{ value: 'and', text: 'And' }, { value: 'or', text: 'Or' }], true);
            window['FwFormField_select'].loadItems(booleanField, [{ value: 'T', text: 'true' }, { value: 'F', text: 'false' }], true);
            $control.find('.datafieldselect').on('change', function () {
                ...
                switch (datatype) {
                    case 'Text':
                        textField.show();
                        window['FwFormField_select'].loadItems(jQuery($control.find('.datafieldcomparison')[0]), textComparisonFields, true);
                        break;
                    case 'Integer':
                    case 'Decimal':
                    case 'Float':
                        textField.show();
                        window['FwFormField_select'].loadItems(jQuery($control.find('.datafieldcomparison')[0]), numericComparisonFields, true);
                        break;
                    case 'True/False':
                    case 'Boolean':
                        booleanField.show();
                        window['FwFormField_select'].loadItems(jQuery($control.find('.datafieldcomparison')[0]), booleanComparisonFields, true);
                        break;
                    case 'Date':
                        dateField.show();
                        window['FwFormField_select'].loadItems(jQuery($control.find('.datafieldcomparison')[0]), dateComparisonFields, true);
                        break;
                }
            })
        ...
        $control.find('.add-query').on('click', function cloneRow() {
            ...
            $newRow.find('.delete-query').on('click', function () {
                ...
                if ($control.find('.query').find('.queryrow').length === 2 && $newRow.next().length !== 0) {
                    $newRow.next().find('.delete-query').removeAttr('style').css('visibility', 'hidden');
                }
                if ($control.find('.query').find('.queryrow').length === 2 && $newRow.prev().length !== 0) {
            ...
            $newRow.appendTo($control.find('.query'));
            if ($control.find('.query').find('.queryrow').length > 1) {
                jQuery($control.find('.query').find('.queryrow')[0]).find('.delete-query').on('click', function () {
        ...
        $control.find('.querysearch').on('click', function (e) {
            let request = FwBrowse.getRequest($control);
            let queryRows = $control.find('.query').find('.queryrow');
            ...
            $control.data('advancedsearchrequest', request);
            e.stopPropagation();
        })
