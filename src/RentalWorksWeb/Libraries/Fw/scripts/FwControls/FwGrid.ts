            ...
            if (FwApplicationTree.isVisibleInSecurityTree(option.securityid)) {
                ...
                if (option.type === 'option') {
                    let $item = jQuery('<div>')
                        ...
                        .appendTo($dropdown)
                        .on('click', (e) => (option as GridMenuOption).action($grid, e))
                    ...
                    let $item = jQuery('<div>')
                        .addClass('dropdown-item')
                        .appendTo($dropdown)
                        .on('click', (e) => this._downloadExcelWorkbook($grid))
            ...
            if (item === 'seperator') {
                ...
                if (FwApplicationTree.isVisibleInSecurityTree(dropDownButtonItem.securityid)) {
                    ...
                    let $item = jQuery('<div>')
                        .addClass('dropdown-item')
                        .appendTo($dropdown)
                        .on('click', (e) => (item as GridMenuDropDownButtonItem).action($grid, e))
        ...
        for (let column of options.columns) {
            if (!column.hidden) {
                ...
                if (column.sort !== false) {
                    ...
                    $column.on('click', (e) => me._columnHeaderClick($grid, e));
