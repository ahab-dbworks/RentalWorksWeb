...
import { FwModuleBase, FwNewRecordToCreate, FwRecordToEdit } from './FwModuleBase';
import { FwLogging } from './FwLogging';
import { FwTestUtils } from './FwTestUtils';

export class FwSaveGridRowResponse {
    saved: boolean;
    errorMessage: string;
    errorFields: string[];
}

export class FwAddGridRowResponse extends FwSaveGridRowResponse { }

export class FwEditGridRowResponse extends FwSaveGridRowResponse { }

export class FwDeleteGridRowResponse {
    deleted: boolean;
    errorMessage: string;
}

export class FwGridRecordToCreate {
    grid: FwGridBase;
    recordToCreate?: FwNewRecordToCreate;
    recordToEdit?: FwRecordToEdit;
}

//---------------------------------------------------------------------------------------
export class FwGridBase {
    ...
    constructor(gridDisplayName: string, gridName: string, gridClass?: string[]) {
        ...
        this.globalScopeRef = FwGlobalScope;
    }
    //---------------------------------------------------------------------------------------
    async clickGridTab() {
        ...
        if (!tabIsActive) {
            FwLogging.logInfo(`Clicking tab ${tabId}`);
            await page.click(`#${tabId}`);
            await FwModuleBase.wait(1500); // wait for the grid to refresh if any
        }
    }
    //---------------------------------------------------------------------------------------
    ...
    async getRecordCount(): Promise<number> {
        FwLogging.logInfo(`About to check number of rows in grid: ${this.gridName}`);
    ...
    async checkForNewButton(): Promise<boolean> {
        ...
        FwLogging.logInfo(`About to check for new button on grid: ${this.gridName}`);
    ...
    async checkForEditAbility(): Promise<boolean> {
        ...
        FwLogging.logInfo(`About to check for Edit ability on grid: ${this.gridName}`);
        ...
        if (foundEditableCell) {
            ...
            try {
                ...
                FwLogging.logInfo(`found row in EDIT mode on grid: ${this.gridName}`);

                let cancelEditModeButtonSelector = `${this.gridSelector} .tablewrapper table tbody tr.editrow .divcancelsaverow i`;
                FwLogging.logInfo(`looking for grid row edit cancel button ${cancelEditModeButtonSelector}`);
                await page.waitForSelector(cancelEditModeButtonSelector);
                FwLogging.logInfo(`found grid row edit cancel button, about to click`);
                await page.click(cancelEditModeButtonSelector);
                FwLogging.logInfo(`clicked grid row edit cancel button`);
    ...
    async checkForDeleteOption(): Promise<boolean> {
        ...
        FwLogging.logInfo(`About to check for delete option on grid: ${this.gridName}`);

        await this.clickGridTab();

        let gridMenuSelector = `${this.gridSelector} .submenubutton i`;
        await page.waitForSelector(gridMenuSelector, { visible: true });
        await FwModuleBase.wait(200); // wait for the grid hamburger to get its events
        await page.click(gridMenuSelector);
        FwLogging.logInfo(`clicked grid menu button on grid: ${this.gridName}`);
        ...
        await page.click(gridMenuSelector);
        await FwModuleBase.wait(200); // wait for the grid hamburger to get its events
        FwLogging.logInfo(`clicked grid menu button to close menu on grid: ${this.gridName}`);
    ...
    async populateRowWithRecord(record: any): Promise<boolean> {
        ...
        for (let key in record) {
            ...
            FwLogging.logInfo(`About to populate grid FieldName: "${fieldToPopulate}"   Value: "${valueToPopulate}"`);
            ...
            switch (datatype) {
                ...
                case 'radio':
                    break;
                case 'validation':
                    FwLogging.logInfo(`about to populate field ${fieldToPopulate}`);
                    ...
                    FwLogging.logInfo(`displayField is ${displayField}`);
                    gridFieldSelector = `${this.gridSelector} .tablewrapper table tbody tr td div[data-browsedatafield="${fieldToPopulate}"] input.text`;
                    FwLogging.logInfo(`gridFieldSelector is ${gridFieldSelector}`);
                    currentValue = await page.$eval(gridFieldSelector, (e: any) => e.value);
                    FwLogging.logInfo(`currentValue is ${currentValue}`);
                    if (currentValue != "") {
                    ...
                    if (valueToPopulate !== 0) {
                        ...
                        FwLogging.logInfo(`validationname is ${validationname}`);
            ...
            if (this.waitAfterInputtingEachCellValueOnNewRow > 0) {
                await FwModuleBase.wait(this.waitAfterInputtingEachCellValueOnNewRow);
            }
        }
        success = true;
        return success;
    ...
    //---------------------------------------------------------------------------------------
    async saveRow(closeUnexpectedErrors: boolean = false): Promise<FwSaveGridRowResponse> {

        let response = new FwSaveGridRowResponse();
        response.saved = false;
        response.errorMessage = "not saved";
        response.errorFields = new Array<string>();

        FwLogging.logInfo(`about to find the Save button on grid: ${this.gridName}`);
        ...
        FwLogging.logInfo(`found the Save button on grid: ${this.gridName}.  About to click.`);
        await page.click(gridRowSaveButtonSelector);

        FwLogging.logInfo(`about to wait for the Please Wait dialog after saving grid: ${this.gridName}.`);
        ...
        FwLogging.logInfo(`Please Wait dialog is gone after saving grid: ${this.gridName}.`);

        FwLogging.logInfo(`about to check for error pop-up after saving grid: ${this.gridName}.`);
        ...
        if (popUp !== undefined) {
            FwLogging.logInfo(`error pop-up found saving grid: ${this.gridName}.`);
            let selector: string = ``;
            let errorMessage = await page.$eval('.advisory', el => el.textContent);
            response.saved = false;
            ...
            FwLogging.logInfo(`${this.gridDisplayName} Record not saved: ${errorMessage}`);
            ...
            const elementHandle = await page.$(selector);
            if (elementHandle != null) {
                FwLogging.logInfo(`Found toaster button, about to click`);
                await elementHandle.click();
            ...
            FwLogging.logInfo(`Found grid row cancel button. about to click`);
            await page.click(gridRowCancelButtonSelector);
        }
        else {
            FwLogging.logInfo(`no error pop-up found saving grid: ${this.gridName}.`);
            response.saved = true;
            response.errorMessage = "";
        }

        if (this.waitAfterSavingToReloadGrid > 0) {
            await FwModuleBase.wait(this.waitAfterSavingToReloadGrid);
        }
    ...
    //---------------------------------------------------------------------------------------
    async addGridRow(record: any, closeUnexpectedErrors: boolean = false): Promise<FwAddGridRowResponse> {

        let response = new FwAddGridRowResponse();
        response.saved = false;
        response.errorMessage = "not saved";
        response.errorFields = new Array<string>();

        FwLogging.logInfo(`About to add a new row to grid: ${this.gridName}`);

        await this.clickGridTab();

        await page.waitForSelector(this.newButtonSelector, { visible: true });
        await FwModuleBase.wait(1000); // wait for the New button to get its events
        await page.click(this.newButtonSelector);
        FwLogging.logInfo(`clicked New button on grid: ${this.gridName}`);

        let gridNewRowSelector = `${this.gridSelector} tbody tr`;
        await page.waitForSelector(gridNewRowSelector);
        FwLogging.logInfo(`found new row on grid: ${this.gridName}`);

        await this.populateRowWithRecord(record);

        if (this.waitBeforeClickingSaveButtonOnNewRow > 0) {
            await FwModuleBase.wait(this.waitBeforeClickingSaveButtonOnNewRow);
        }
    ...
    //---------------------------------------------------------------------------------------
    async editGridRow(rowToEdit: number, record: any, closeUnexpectedErrors: boolean = false): Promise<FwEditGridRowResponse> {

        let response = new FwEditGridRowResponse();
        response.saved = false;
        response.errorMessage = "not saved";
        response.errorFields = new Array<string>();

        FwLogging.logInfo(`About to edit row in grid: ${this.gridName}`);

        await this.clickGridTab();

        //let editableCellSelector = `${this.gridSelector} .tablewrapper table tbody tr:nth-child(${rowToEdit}) .editablefield`;
        //FwLogging.logInfo(`editableCellSelector: ${editableCellSelector}`);
        //await page.waitForSelector(editableCellSelector);
        //FwLogging.logInfo(`found editable cell on grid: ${this.gridName}`);
        //await ModuleBase.wait(200); // wait for the grid row to get its events
        //
        //FwLogging.logInfo(`about to click editable cell on grid: ${this.gridName}`);
        ...
        if (this.waitBeforeClickingSaveButtonOnEditRow > 0) {
            await FwModuleBase.wait(this.waitBeforeClickingSaveButtonOnEditRow);
        }
    ...
    //---------------------------------------------------------------------------------------
    async deleteGridRow(rowToDelete?: number, closeUnexpectedErrors?: boolean): Promise<FwDeleteGridRowResponse> {
        let response = new FwDeleteGridRowResponse();
        ...
        FwLogging.logInfo(`About to delete row from grid: ${this.gridName}`);

        await this.clickGridTab();

        await FwModuleBase.wait(this.waitForGridSubMenuToGetEvents);  // wait for the grid sub menu to get its events
        let gridContextMenuSelector = `${this.gridSelector} .tablewrapper table tbody tr:nth-child(${rowToDelete}) .browsecontextmenu i`;
        FwLogging.logInfo(`About to wait for row context menu: ${gridContextMenuSelector}`);
        await page.waitForSelector(gridContextMenuSelector);
        await page.click(gridContextMenuSelector);
        FwLogging.logInfo(`clicked the row context menu`);
        ...
        if (popupText.includes('Delete Record')) {
            ...
            if (afterDeleteMsg.includes('Error') || afterDeleteMsg.includes('Not Found')) {
                FwLogging.logInfo(`${this.gridDisplayName} Record not deleted: ${afterDeleteMsg}`);
                response.deleted = false;
                ...
                FwLogging.logInfo(`${this.gridDisplayName} Record deleted: ${afterDeleteMsg}`);
    ...
    async getRecordRowIndex(record: any): Promise<number> {
        ...
        FwLogging.logInfo(`About to try to find row for object: ${JSON.stringify(record)}`);

        await this.clickGridTab();

        FwLogging.logInfo(`About to wait for grid: ${this.gridSelector}`);
        ...
        FwLogging.logInfo(`Found ${rowCount} rows in grid: ${this.gridSelector}`);
        ...
        for (let r = 1; r <= rowCount; r++) {
            ...
            for (let key in record) {
                ...
                if (valueToFind.toString().toUpperCase().includes("GLOBALSCOPE.")) {
                    valueToFind = FwTestUtils.getGlobalScopeValue(valueToFind, this.globalScopeRef);
                }
                ...
                FwLogging.logInfo(`Found value of ${cellValue} in field ${fieldToFind} in grid: ${this.gridSelector}`);
