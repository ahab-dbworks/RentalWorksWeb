var FwMultiSelectValidationClass = (function () {
    ...
    FwMultiSelectValidationClass.prototype.init = function ($control, validationName, $valuefield, $searchfield, $btnvalidate) {
        ...
        if (hasselectall)
            $searchfield.attr('placeholder', 'ALL');
        $searchfield;
        ...
        $browse.data('ondatabind', function (request) {
            ...
            if ((typeof boundfields != 'undefined') && (boundfields.length > 0)) {
                ...
                for (var i = 0; i < boundfields.length; i++) {
                    ...
                    if ($boundfield.length == 0)
                        throw 'Unable to find boundfield "' + boundid + '" for ' + $control.attr('data-caption');
                    boundid = FwFormField.getValue2($boundfield);
                    if ((typeof boundid === 'undefined') || (boundid === ''))
                        throw 'Please select "' + $boundfield.attr('data-caption') + '" first.';
            ...
            FwServices.validation.method(request, validationName, 'Browse', $browse, function (response) {
                try {
