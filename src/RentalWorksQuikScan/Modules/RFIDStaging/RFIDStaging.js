...
RFIDStaging.getModuleScreen = function(viewModel, properties) {
    ...
    combinedViewModel.htmlPageBody = Mustache.render(jQuery('#tmpl-rfidstaging').html(), combinedViewModel);
    screen = {
        isscanning: false,
    ...
    var $stageditems = FwMobileMasterController.tabcontrols.addtab('Staged Items', false);
    ...
    screen.$view
        ...
        .on('blur', '.txtscanagain', function() {
            ...
            $txt.removeAttr('data-oldvalue');
            localStorage.setItem('rwqs_rfidstaging_scanagain', $txt.val());
        })
    ;
    ...
    screen.getOrderId = function() {
        return properties.webSelectOrder.orderId;
    };
    ...
    screen.processBatch = function(tags) {
        ...
        RwServices.call("RfidStaging", "ProcessBatch", request, function(response) {
            try {
    ...
    screen.addItemToOrder = function($item) {
        ...
        RwServices.call("RfidStaging", "AddItemToOrder", request, function(response) {
            try {
                screen.setStagedQty(screen.getStagedQty() + 1);
    ...
    screen.addCompleteToOrder = function($item) {
        ...
        RwServices.call("RfidStaging", "AddCompleteToOrder", request, function(response) {
            try {
                screen.setStagedQty(screen.getStagedQty() + 1);
                screen.handleExceptionResponse(response, $item);
            } catch(ex) {
    ...
    screen.overrideAvailabilityConflict = function($item) {
        ...
        RwServices.call("RfidStaging", "OverrideAvailabilityConflict", request, function(response) {
            try {
                screen.setStagedQty(screen.getStagedQty() + 1);
                screen.handleExceptionResponse(response, $item);
            } catch(ex) {
    ...
    screen.transferItemInRepair = function($item) {
        ...
        RwServices.call("RfidStaging", "TransferItemInRepair", request, function(response) {
            try {
                screen.setStagedQty(screen.getStagedQty() + 1);
                screen.handleExceptionResponse(response, $item);
            } catch(ex) {
    ...
    screen.releaseFromRepair = function($item) {
        ...
        RwServices.call("RfidStaging", "ReleaseFromRepair", request, function(response) {
            try {
                screen.setStagedQty(screen.getStagedQty() + 1);
                screen.handleExceptionResponse(response, $item);
            } catch(ex) {
    ...
    screen.clearException = function($item) {
        ...
        RwServices.call("RfidStaging", "ClearException", request, function(response) {
            try {
                screen.setTotalQty(screen.getTotalQty() - 1);
                screen.handleExceptionResponse(response, $item);
            } catch(ex) {
                FwFunc.showError(ex);
            }
    ...
    screen.clearAllExceptions = function() {
        ...
        RwServices.call("RfidStaging", "ClearAllExceptions", request, function(response) {
            try {
                var totalQty = screen.getTotalQty();
                var exceptionQty = screen.getExceptionQty();
                totalQty = totalQty - exceptionQty;
                screen.setTotalQty(totalQty);
                screen.setExceptionQty(0);
                screen.loadExceptions(response.funcscannedtagexception);
            } catch(ex) {
                FwFunc.showError(ex);
            }
    ...
    screen.removeException = function($item) {
        ...
        var newqty = screen.getExceptionQty() - 1;
        screen.setExceptionQty(newqty);
        if (newqty === 0) {
    ...
    screen.getTotalQty = function () {
        var qty = parseInt(screen.$view.find('.batchqtyvalue').html());
        return qty;
    };

    screen.setTotalQty = function (qty) {
        screen.$view.find('.batchqtyvalue').html(qty);
    };

    screen.getStagedQty = function () {
        var qty = parseInt(screen.$view.find('.processedcount').html());
        return qty;
    };

    screen.setStagedQty = function (qty) {
        screen.$view.find('.processedcount').html(qty)
        screen.$view.find('.processqtyvalue').html(qty);
    };

    screen.getExceptionQty = function () {
        var qty = parseInt(screen.$view.find('.exceptioncount').html());
        return qty;
    };

    screen.setExceptionQty = function (qty) {
        screen.$view.find('.exceptioncount').html(qty)
        screen.$view.find('.exceptionqtyvalue').html(qty);
    };

    screen.getPendingQty = function () {
        var qty = parseInt(screen.$view.find('.pendingitemscount').html());
        return qty;
    };

    screen.setPendingQty = function (qty) {
        screen.$view.find('.pendingitemscount').html(qty);
    };
    ...
    screen.getExceptions = function() {
        ...
        RwServices.call("RfidStaging", "GetExceptions", request, function(response) {
            try {
                screen.loadExceptions(response.funcscannedtagexception);
            } catch(ex) {
                FwFunc.showError(ex);
    ...
    screen.loadExceptions = function(dt) {
        ...
        if (dt.Rows.length > 0) {
            ...
            screen.setExceptionQty(dt.Rows.length);
            setTimeout(function() {
                try {
    ...
    screen.getPendingItems = function() {
        ...
        RwServices.call("RfidStaging", "GetPendingItems", request, function(response) {
            try {
                ...
                screen.setPendingQty(pendingcount);
            } catch(ex) {
                FwFunc.showError(ex);
            }
        });
    ...
    screen.getStagedItems = function() {
        var request = {
            orderid: screen.getOrderId()
        };
        RwServices.call("RfidStaging", "GetStagedItems", request, function(response) {
            try {
    ...
    screen.unstageItem = function($item) {
        ...
        RwServices.call("RfidStaging", "UnstageItem", request, function(response) {
            try {
                switch(response.status) {
                    case 0:
                        $item.remove();
