var QCClass = (function () {
    ...
    QCClass.prototype.getQCScreen = function (viewModel, properties) {
        ...
        if (program.hasHfRfidApplicationOption === true) {
            ...
            if (typeof window.DTDevices !== 'undefined' && typeof window.DTDevices.rfInitWithFieldGain === 'function') {
                DTDevices.registerListener('rfCardDetected', 'rfCardDetected_applicationjs', function (returnUid, returnType) {
        ...
        $search.on('change', '.fwmobilecontrol-value', function () {
            ...
            if ($this.val() !== '') {
                ...
                RwServices.callMethod('QC', 'ItemScan', request, function (response) {
                    ...
                    if (response.qcitem.status === 0)
                        $item.showscreen(response);
                });
            }
        });
