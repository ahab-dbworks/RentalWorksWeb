                    ...
                    case RwConstants.ModuleTypes.Order:
                        switch (searchmode)
                        {
                            case "orderno":
                                ...
                                select.Add("               from masteritem mi with (nolock)");
                                select.Add("               where mi.orderid     = o.orderid");
                                select.Add("                 and mi.rectype in ('R', 'S')");
                                select.Add("                 and mi.warehouseid = @warehouseid)");
                                select.Add("                 or (not exists (select *");
                                select.Add("                                 from masteritem mi with (nolock)");
                                select.Add("                                 where mi.orderid = o.orderid");
                                select.Add("                                   and mi.rectype in ('R', 'S'))");
                                select.Add("                                   and o.warehouseid = @warehouseid))");
                                select.Add("                                   and (o.rental = 'T' or o.sales  = 'T')");
                                select.Add("                                   and o.status not in ('COMPLETE','CLOSED','SNAPSHOT','CANCELLED')");
