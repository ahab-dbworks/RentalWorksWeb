...
namespace RentalWorksQuikScan.Modules
{
    class ReceiveOnSet
    {
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void POSearch(dynamic request, dynamic response, dynamic session)
        {
            ...
            qry.AddColumn("acquire",   false, FwJsonDataTableColumn.DataTypes.Date);
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void GetPOReceiveContractID(dynamic request, dynamic response, dynamic session)
        {
            FwSqlCommand qry;
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void LoadProductions(dynamic request, dynamic response, dynamic session)
        {
            FwSqlCommand qry;
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void LoadSets(dynamic request, dynamic response, dynamic session)
        {
            FwSqlCommand qry;
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void NewOrder(dynamic request, dynamic response, dynamic session)
        {
            dynamic userLocation;
            FwSqlCommand qry2 = new FwSqlCommand(FwSqlConnection.RentalWorks);
            ...
            using (FwSqlCommand qry = new FwSqlCommand(FwSqlConnection.RentalWorks, "dbo.receiveonsetneworder"))
            {
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void POReceive(dynamic request, dynamic response, dynamic session)
        {
            ...
            qry = new FwSqlCommand(FwSqlConnection.RentalWorks, "dbo.receiveonsetporeceive");
            qry.AddParameter("@poid",              request.orderid);
        ...
        //---------------------------------------------------------------------------------------------
        [FwJsonServiceMethod]
        public static void POReceiveImage(dynamic request, dynamic response, dynamic session)
        {
            string[] images;
            byte[] image;
            bool hasImages;
            ...
            if (hasImages && (request.images.Length > 0))
            {
                ...
                for (int i = 0; i < images.Length; i++)
                {
                    ...
                    ReceiveOnSetPOReceiveImage(conn:         FwSqlConnection.RentalWorks,
                                               poid:         request.poid,
        ...
        //----------------------------------------------------------------------------------------------------
        private static void ReceiveOnSetPOReceiveImage(FwSqlConnection conn, string poid, string masteritemid, string barcode, byte[] image)
        {
            int width  = 0,
                height = 0;

            using (FwSqlCommand qry = new FwSqlCommand(conn, "dbo.receiveonsetporeceiveimage"))
            {
                qry.AddParameter("@poid",              poid);
                qry.AddParameter("@masteritemid",      masteritemid);
                qry.AddParameter("@barcode",           barcode);
                qry.AddParameter("@image",             FwGraphics.ConvertToJpg(image, ref width, ref height));
        ...
        public static void CreateContract(dynamic request, dynamic response, dynamic session)
        {
            ...
            qry = new FwSqlCommand(FwSqlConnection.RentalWorks, "dbo.receiveonsetcreatecontract");
            qry.AddParameter("@poid",               request.poid);
            qry.AddParameter("@receivecontractid",  request.receivecontractid);
            qry.AddParameter("@orderid",            request.orderid);
            qry.AddParameter("@signatureimage",     FwGraphics.ConvertToJpg(image, ref width, ref height));
