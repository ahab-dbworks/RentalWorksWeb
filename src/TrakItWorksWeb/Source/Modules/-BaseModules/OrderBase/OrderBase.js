class OrderBase {
    constructor() {
        this.CachedOrderTypes = {};
    }
    renderFrames($form, cachedId, period) {
    ...
    dynamicColumns($form, orderTypeId) {
        let self = this;
        let orderType, $rentalGrid = $form.find('.rentalgrid [data-name="OrderItemGrid"]'), fields = jQuery($rentalGrid).find('thead tr.fieldnames > td.column > div.field'), fieldNames = [];
    ...
    columnLogic($form, data) {
        let $rentalGrid = $form.find('.rentalgrid [data-name="OrderItemGrid"]'), $salesGrid = $form.find('.salesgrid [data-name="OrderItemGrid"]'), $laborGrid = $form.find('.laborgrid [data-name="OrderItemGrid"]'), $miscGrid = $form.find('.miscgrid [data-name="OrderItemGrid"]'), $usedSaleGrid = $form.find('.usedsalegrid [data-name="OrderItemGrid"]'), $lossDamageGrid = $form.find('.lossdamagegrid [data-name="OrderItemGrid"]'), $combinedGrid = $form.find('.combinedgrid [data-name="OrderItemGrid"]'), rate = FwFormField.getValueByDataField($form, 'RateType');
    ...
    ;
    calculateOrderItemGridTotals($form, gridType, totals) {
    ...
    ;
    deliveryTypeAddresses($form, event) {
        let $element;
        $element = jQuery(event.currentTarget);
    ...
    getWarehouseAddress($form, prefix) {
    ...
    fillDeliveryAddressFieldsforDeal($form, prefix, response) {
        if (!response) {
            const DEALID = FwFormField.getValueByDataField($form, 'DealId');
    ...
    afterLoad($form) {
        ...
        if (!FwFormField.getValueByDataField($form, 'Sales')) {
            $form.find('[data-type="tab"][data-caption="Sales"]').hide();
        }
        if (!FwFormField.getValueByDataField($form, 'Miscellaneous')) {
            $form.find('[data-type="tab"][data-caption="Miscellaneous"]').hide();
        }
        if (!FwFormField.getValueByDataField($form, 'Labor')) {
            $form.find('[data-type="tab"][data-caption="Labor"]').hide();
        }
        if (!FwFormField.getValueByDataField($form, 'RentalSale')) {
