...
namespace WebApi.Modules.Agent.Vendor
{
    ...
    public class VendorLogic : AppBusinessLogic
    {
        ...
        private string tmpVendorNumber = string.Empty;
        ...
        [FwLogicProperty(Id: "js05wMRL0tLXo")]
        public string RemitAttention2 { get { return vendor.RemitAttention2; } set { vendor.RemitAttention2 = value; } }
        ...
        public void OnBeforeValidate(object sender, BeforeValidateEventArgs e)
        {
            if (e.SaveMode.Equals(TDataRecordSaveMode.smInsert))
            {
                ...
                {
                    tmpVendorNumber = AppFunc.GetNextIdAsync(AppConfig).Result;
                    VendorNumber = tmpVendorNumber;
                }
            }
        }
        //------------------------------------------------------------------------------------ 
        public void OnBeforeSave(object sender, BeforeSaveEventArgs e)
        {
            if (e.SaveMode.Equals(TDataRecordSaveMode.smInsert))
            {
                if ((string.IsNullOrEmpty(VendorNumber)) || (VendorNumber.Equals(tmpVendorNumber)))
                {
                    VendorNumber = AppFunc.GetNextSystemCounterAsync(AppConfig, UserSession, "vendorno", e.SqlConnection).Result;
                }
            }
        }
        //------------------------------------------------------------------------------------ 
