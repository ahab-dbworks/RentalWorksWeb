            ...
            using (FwSqlConnection conn = new FwSqlConnection(this.ApplicationConfig.DatabaseSettings.ConnectionString))
            {
                if ((mode == "ORDER") || (mode == "DEAL") || (mode == "EVENT"))
                {
                    response.status = await this.AppData.TimeLogSubmitAsync(conn: conn,
                                                              ...
                                                              orderid: request.orderid,
                                                              masteritemid: request.masteritemid,
                                                              starttime: FwConvert.ToShortTime(FwConvert.ToDateTime(request.starttime)),
                                                              stoptime: FwConvert.ToShortTime(FwConvert.ToDateTime(request.stoptime)),
                                                              break1starttime: (request.break1starttime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break1starttime)) : "",
                                                              break1stoptime: (request.break1stoptime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break1stoptime)) : "",
                                                              break2starttime: (request.break2starttime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break2starttime)) : "",
                                                              break2stoptime: (request.break2stoptime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break2stoptime)) : "",
                                                              break3starttime: (request.break3starttime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break3starttime)) : "",
                                                              break3stoptime: (request.break3stoptime != "") ? FwConvert.ToShortTime(FwConvert.ToDateTime(request.break3stoptime)) : "",
                                                              notes: request.notes,
