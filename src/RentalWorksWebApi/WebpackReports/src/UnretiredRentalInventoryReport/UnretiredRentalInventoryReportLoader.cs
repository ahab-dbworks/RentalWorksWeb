...
namespace WebApi.Modules.Reports.UnretiredRentalInventoryReport
{
    public class UnretiredRentalInventoryReportLoader : AppDataLoadRecord
    {
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> RunReportAsync(UnretiredRentalInventoryReportRequest request)
        {
            FwJsonDataTable dt = null;
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                using (FwSqlCommand qry = new FwSqlCommand(conn, "getunretireditemsrpt", this.AppConfig.DatabaseSettings.ReportTimeout))
                {
                    qry.AddParameter("@availfor", SqlDbType.Text, ParameterDirection.Input, RwConstants.INVENTORY_AVAILABLE_FOR_RENT);
                    qry.AddParameter("@unretiredfromdate", SqlDbType.Date, ParameterDirection.Input, request.FromDate);
                    qry.AddParameter("@unretiredtodate", SqlDbType.Date, ParameterDirection.Input, request.ToDate);
