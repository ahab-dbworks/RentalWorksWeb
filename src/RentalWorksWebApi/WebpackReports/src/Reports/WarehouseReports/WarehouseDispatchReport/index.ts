...
export class WarehouseDispatchReport extends WebpackReport {

    renderReport(apiUrl: string, authorizationHeader: string, parameters: any): void {
        try {
            super.renderReport(apiUrl, authorizationHeader, parameters);
            Ajax.post<DataTable>(`${apiUrl}/api/v1/warehousedispatchreport/runreport`, authorizationHeader, parameters)
                .then((response: DataTable) => {
                    ...
                    data.Report = 'Warehouse Dispatch Report';
...
(<any>window).report = new WarehouseDispatchReport();