...
namespace WebApi.Modules.Reports.ContractReports.ExchangeContractReport
{
    ...
    public class ExchangeContractItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(ExchangeContractReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class ExchangeContractReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<ExchangeContractReportLoader> RunReportAsync(ExchangeContractReportRequest request)
        {

            Task<ExchangeContractReportLoader> taskExchangeContract;
            Task<FwJsonDataTable> taskExchangeContractItems;

            ExchangeContractReportLoader ExchangeContract = null;
            ExchangeContractItemReportLoader ExchangeContractItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskExchangeContract = qry.QueryToTypedObjectAsync<ExchangeContractReportLoader>();
                    ...
                    ExchangeContractItems = new ExchangeContractItemReportLoader();
                    ExchangeContractItems.SetDependencies(AppConfig, UserSession);
                    taskExchangeContractItems = ExchangeContractItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskExchangeContract, taskExchangeContractItems });

                    ExchangeContract = taskExchangeContract.Result;

                    if (ExchangeContract != null)
                    {
                        ExchangeContract.Items = taskExchangeContractItems.Result;
                    }
