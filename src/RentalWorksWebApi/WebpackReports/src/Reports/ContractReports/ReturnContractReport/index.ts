...
export class ReturnContractReport extends WebpackReport {
        ...
        try {
            ...
            Ajax.get<DataTable>(`${apiUrl}/api/v1/logosettings/1`, authorizationHeader)
                .then((response: DataTable) => {
                    ...
                    Ajax.post<ReturnContract>(`${apiUrl}/api/v1/returncontractreport/runreport`, authorizationHeader, parameters)
                        .then((response: ReturnContract) => {
                            const data: any = response;
                            data.Items = DataTable.toObjectList(response.Items);
                            data.PrintTime = `Printed on ${moment().format('MM/DD/YYYY')} at ${moment().format('h:mm:ss A')}`;
                            data.System = 'RENTALWORKS';
                            data.Report = 'RETURN CONTRACT';
    ...
    renderFooterHtml(model: ReturnContract): string {
        this.footerHtml = hbFooter(model);
        return this.footerHtml;
    }
}

(<any>window).report = new ReturnContractReport();
