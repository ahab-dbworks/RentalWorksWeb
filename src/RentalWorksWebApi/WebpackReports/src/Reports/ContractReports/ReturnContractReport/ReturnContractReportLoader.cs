...
namespace WebApi.Modules.Reports.ContractReports.ReturnContractReport
{
    ...
    public class ReturnContractItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(ReturnContractReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class ReturnContractReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        public async Task<ReturnContractReportLoader> RunReportAsync(ReturnContractReportRequest request)
        {
            ...
            Task<ReturnContractReportLoader> taskReturnContract;
            Task<FwJsonDataTable> taskReturnContractItems;

            ReturnContractReportLoader ReturnContract = null;
            ReturnContractItemReportLoader ReturnContractItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskReturnContract = qry.QueryToTypedObjectAsync<ReturnContractReportLoader>();
                    ...
                    ReturnContractItems = new ReturnContractItemReportLoader();
                    ReturnContractItems.SetDependencies(AppConfig, UserSession);
                    taskReturnContractItems = ReturnContractItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskReturnContract, taskReturnContractItems });

                    ReturnContract = taskReturnContract.Result;

                    if (ReturnContract != null)
                    {
                        ReturnContract.Items = taskReturnContractItems.Result;
                    }
