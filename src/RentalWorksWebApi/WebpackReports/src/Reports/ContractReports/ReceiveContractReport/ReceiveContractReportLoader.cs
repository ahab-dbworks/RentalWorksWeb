...
namespace WebApi.Modules.Reports.ContractReports.ReceiveContractReport
{
    ...
    public class ReceiveContractItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(ReceiveContractReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class ReceiveContractReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        public async Task<ReceiveContractReportLoader> RunReportAsync(ReceiveContractReportRequest request)
        {
            ...
            Task<ReceiveContractReportLoader> taskReceiveContract;
            Task<FwJsonDataTable> taskReceiveContractItems;

            ReceiveContractReportLoader ReceiveContract = null;
            ReceiveContractItemReportLoader ReceiveContractItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskReceiveContract = qry.QueryToTypedObjectAsync<ReceiveContractReportLoader>();
                    ...
                    ReceiveContractItems = new ReceiveContractItemReportLoader();
                    ReceiveContractItems.SetDependencies(AppConfig, UserSession);
                    taskReceiveContractItems = ReceiveContractItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskReceiveContract, taskReceiveContractItems });

                    ReceiveContract = taskReceiveContract.Result;

                    if (ReceiveContract != null)
                    {
                        ReceiveContract.Items = taskReceiveContractItems.Result;
                    }
