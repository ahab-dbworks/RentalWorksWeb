...
namespace WebApi.Modules.Reports.ContractReports.LostContractReport
{
    ...
    public class LostContractItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(LostContractReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class LostContractReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        public async Task<LostContractReportLoader> RunReportAsync(LostContractReportRequest request)
        {
            ...
            Task<LostContractReportLoader> taskLostContract;
            Task<FwJsonDataTable> taskLostContractItems;

            LostContractReportLoader LostContract = null;
            LostContractItemReportLoader LostContractItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskLostContract = qry.QueryToTypedObjectAsync<LostContractReportLoader>();
                    ...
                    LostContractItems = new LostContractItemReportLoader();
                    LostContractItems.SetDependencies(AppConfig, UserSession);
                    taskLostContractItems = LostContractItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskLostContract, taskLostContractItems });

                    LostContract = taskLostContract.Result;

                    if (LostContract != null)
                    {
                        LostContract.Items = taskLostContractItems.Result;
                    }
