...
namespace WebApi.Modules.Reports.ContractReports.TransferReceiptReport
{
    ...
    public class TransferReceiptItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(TransferReceiptReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class TransferReceiptReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        public async Task<TransferReceiptReportLoader> RunReportAsync(TransferReceiptReportRequest request)
        {
            ...
            Task<TransferReceiptReportLoader> taskTransferReceipt;
            Task<FwJsonDataTable> taskTransferReceiptItems;

            TransferReceiptReportLoader TransferReceipt = null;
            TransferReceiptItemReportLoader TransferReceiptItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskTransferReceipt = qry.QueryToTypedObjectAsync<TransferReceiptReportLoader>();
                    ...
                    TransferReceiptItems = new TransferReceiptItemReportLoader();
                    TransferReceiptItems.SetDependencies(AppConfig, UserSession);
                    taskTransferReceiptItems = TransferReceiptItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskTransferReceipt, taskTransferReceiptItems });

                    TransferReceipt = taskTransferReceipt.Result;

                    if (TransferReceipt != null)
                    {
                        TransferReceipt.Items = taskTransferReceiptItems.Result;
                    }
