...
namespace WebApi.Modules.Reports.ContractReports.TransferManifestReport
{
    ...
    public class TransferManifestItemReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        ...
        //------------------------------------------------------------------------------------ 
        public async Task<FwJsonDataTable> LoadItems(TransferManifestReportRequest request)
        {
            useWithNoLock = false;
    ...
    public class TransferManifestReportLoader : AppReportLoader
    {
        //------------------------------------------------------------------------------------ 
        [FwSqlDataField(calculatedColumnSql: "'detail'", modeltype: FwDataTypes.Text, isVisible: false)]
        public string RowType { get; set; }
        ...
        public async Task<TransferManifestReportLoader> RunReportAsync(TransferManifestReportRequest request)
        {
            ...
            Task<TransferManifestReportLoader> taskTransferManifest;
            Task<FwJsonDataTable> taskTransferManifestItems;

            TransferManifestReportLoader TransferManifest = null;
            TransferManifestItemReportLoader TransferManifestItems = null;
            ...
            using (FwSqlConnection conn = new FwSqlConnection(AppConfig.DatabaseSettings.ConnectionString))
            {
                ...
                using (FwSqlCommand qry = new FwSqlCommand(conn, AppConfig.DatabaseSettings.ReportTimeout))
                {
                    ...
                    //taskTransferManifest = qry.QueryToTypedObjectAsync<TransferManifestReportLoader>();
                    ...
                    TransferManifestItems = new TransferManifestItemReportLoader();
                    TransferManifestItems.SetDependencies(AppConfig, UserSession);
                    taskTransferManifestItems = TransferManifestItems.LoadItems(request);

                    await Task.WhenAll(new Task[] { taskTransferManifest, taskTransferManifestItems });

                    TransferManifest = taskTransferManifest.Result;

                    if (TransferManifest != null)
                    {
                        TransferManifest.Items = taskTransferManifestItems.Result;
                    }
