class ReturnToVendor {
    constructor() {
        ...
        this.caption = Constants.Modules.Home.ReturnToVendor.caption;
        this.nav = Constants.Modules.Home.ReturnToVendor.nav;
        this.id = Constants.Modules.Home.ReturnToVendor.id;
    }
    getModuleScreen() {
        var screen = {};
    ...
    getSuspendedSessions($form) {
        ...
        if (showSuspendedSessions != "false") {
            FwAppData.apiMethod(true, 'GET', 'api/v1/purchaseorder/returnsuspendedsessionsexist', null, FwServices.defaultTimeout, response => {
    ...
    getItems($form) {
        ...
        $form.find('[data-datafield="PurchaseOrderId"]').data('onchange', $tr => {
            ...
            FwAppData.apiMethod(true, 'POST', 'api/v1/purchaseorder/startreturncontract', request, FwServices.defaultTimeout, function onSuccess(response) {
                let contractId = response.ContractId, $pOReturnItemGridControl, max = 9999;
