class ReceiveFromVendor {
    constructor() {
        ...
        this.caption = Constants.Modules.Home.ReceiveFromVendor.caption;
        this.nav = Constants.Modules.Home.ReceiveFromVendor.nav;
        this.id = Constants.Modules.Home.ReceiveFromVendor.id;
    }
    getModuleScreen() {
        var screen = {};
    ...
    getSuspendedSessions($form) {
        ...
        if (showSuspendedSessions != "false") {
            FwAppData.apiMethod(true, 'GET', 'api/v1/purchaseorder/receivesuspendedsessionsexist', null, FwServices.defaultTimeout, response => {
    ...
    getItems($form) {
        $form.find('[data-datafield="PurchaseOrderId"]').data('onchange', $tr => {
            ...
            FwAppData.apiMethod(true, 'POST', 'api/v1/purchaseorder/startreceivecontract', request, FwServices.defaultTimeout, function onSuccess(response) {
                let contractId = response.ContractId, $receiveItemsGridControl, max = 9999;
