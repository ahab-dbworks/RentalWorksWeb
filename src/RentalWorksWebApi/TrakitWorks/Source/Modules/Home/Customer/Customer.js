routes.push({ pattern: /^module\/customer$/, action: function (match) { return CustomerController.getModuleScreen(); } });
routes.push({ pattern: /^module\/customer\/(\S+)\/(\S+)/, action: function (match) { var filter = { 'datafield': match[1], 'search': match[2].replace(/%20/g, ' ').replace(/%2f/g, '/') }; return CustomerController.getModuleScreen(filter); } });
class Customer {
    constructor() {
        this.Module = 'Customer';
        this.apiurl = 'api/v1/customer';
        this.caption = Constants.Modules.Home.Customer.caption;
        this.nav = Constants.Modules.Home.Customer.nav;
        this.id = Constants.Modules.Home.Customer.id;
    }
    ...
    openForm(mode) {
        ...
        $submoduleDealBrowse.find('div.btn[data-type="NewMenuBarButton"]').on('click', function () {
            var $dealForm, controller, $browse, dealFormData = {};
    ...
    loadAudit($form) {
        var uniqueid = FwFormField.getValueByDataField($form, 'CustomerId');
        FwModule.loadAudit($form, uniqueid);
    }
    renderGrids($form) {
        var nameCustomerResaleGrid = 'CompanyResaleGrid';
        var $companyResaleGrid = $companyResaleGrid = $form.find('div[data-grid="' + nameCustomerResaleGrid + '"]');
        var $companyResaleGridControl = FwBrowse.loadGridFromTemplate(nameCustomerResaleGrid);
        ...
        var nameCustomerNoteGrid = 'CustomerNoteGrid';
        var $customerNoteGrid = $customerNoteGrid = $form.find('div[data-grid="' + nameCustomerNoteGrid + '"]');
        var $customerNoteGridControl = FwBrowse.loadGridFromTemplate(nameCustomerNoteGrid);
        ...
        var nameCompanyTaxGrid = 'CompanyTaxOptionGrid';
        var $companyTaxGrid = $companyTaxGrid = $form.find('div[data-grid="' + nameCompanyTaxGrid + '"]');
        var $companyTaxControl = FwBrowse.loadGridFromTemplate(nameCompanyTaxGrid);
    ...
    afterLoad($form) {
        var $customerResaleGrid = $form.find('[data-name="CompanyResaleGrid"]');
        var $customerNoteGrid = $form.find('[data-name="CustomerNoteGrid"]');
        var $companyTaxGrid = $form.find('[data-name="CompanyTaxOptionGrid"]');
