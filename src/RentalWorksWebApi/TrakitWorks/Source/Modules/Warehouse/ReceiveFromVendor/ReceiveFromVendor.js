class ReceiveFromVendor {
    constructor() {
        ...
        this.apiurl = 'api/v1/receivefromvendor';
        this.caption = Constants.Modules.Warehouse.children.ReceiveFromVendor.caption;
        this.nav = Constants.Modules.Warehouse.children.ReceiveFromVendor.nav;
        this.id = Constants.Modules.Warehouse.children.ReceiveFromVendor.id;
    }
    addFormMenuItems(options) {
        options.hasSave = false;
        FwMenu.addFormMenuButtons(options);
        FwMenu.addSubMenuItem(options.$groupOptions, 'Cancel Receive From Vendor', '', (e) => {
            try {
    ...
    getSuspendedSessions($form) {
        ...
        if (showSuspendedSessions != "false") {
            FwAppData.apiMethod(true, 'GET', `api/v1/purchaseorder/receivesuspendedsessionsexist?warehouseId=${warehouse.warehouseid}`, null, FwServices.defaultTimeout, response => {
                if (response) {
        ...
        $form.find('[data-datafield="PurchaseOrderId"]').data('onchange', $tr => {
            ...
            if (contractId.length === 0) {
                ...
                FwAppData.apiMethod(true, 'POST', 'api/v1/purchaseorder/startreceivecontract', request, FwServices.defaultTimeout, function onSuccess(response) {
                    let contractId = response.ContractId, $receiveItemsGridControl;
                    FwFormField.setValueByDataField($form, 'ContractId', contractId);
                    $form.find('.suspendedsession').hide();
                    $form.find('div.caption:contains(Cancel Receive From Vendor)').parent().attr('data-enabled', 'true');
    ...
    addButtonMenu($form) {
        ...
        let $createContract = FwMenu.generateButtonMenuOption('Create Contract'), $createContractAndAssignBarCodes = FwMenu.generateButtonMenuOption('Create Contract and Assign Bar Codes');
        ...
        $createContractAndAssignBarCodes.on('click', e => {
            ...
            let request = {};
            const contractId = FwFormField.getValueByDataField($form, 'ContractId');
            FwAppData.apiMethod(true, 'POST', `api/v1/purchaseorder/completereceivecontract/${contractId}`, request, FwServices.defaultTimeout, response => {
                let contractInfo = {}, $contractForm, $assignBarCodesForm;
...
//# sourceMappingURL=ReceiveFromVendor.js.map