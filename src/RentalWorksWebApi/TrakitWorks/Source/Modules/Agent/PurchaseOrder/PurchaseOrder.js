routes.push({ pattern: /^module\/purchaseorder$/, action: function (match) { return PurchaseOrderController.getModuleScreen(); } });
routes.push({ pattern: /^module\/purchaseorder\/(\w+)\/(\S+)/, action: function (match) { var filter = { datafield: match[1], search: match[2] }; return PurchaseOrderController.getModuleScreen(filter); } });
class PurchaseOrder {
    constructor() {
        this.Module = 'PurchaseOrder';
        this.apiurl = 'api/v1/purchaseorder';
        this.caption = Constants.Modules.Agent.children.PurchaseOrder.caption;
        this.nav = Constants.Modules.Agent.children.PurchaseOrder.nav;
        this.id = Constants.Modules.Agent.children.PurchaseOrder.id;
    ...
    addFormMenuItems(options) {
        ...
        FwMenu.addSubMenuItem(options.$groupOptions, 'Assign Barcodes', 'cCF4flMdl36v', (e) => {
            try {
        ...
        FwMenu.addSubMenuItem(options.$groupOptions, 'Receive From Vendor', 'Ib1R4j9P0d9E', (e) => {
            try {
        ...
        FwMenu.addSubMenuItem(options.$groupOptions, 'Return To Vendor', 'R645XpKYOSX1', (e) => {
            try {
        ...
        FwMenu.addSubMenuItem(options.$groupOptions, 'Search', '', (e) => {
            try {
    ...
    afterLoad($form) {
        ...
        if (!FwFormField.getValueByDataField($form, 'Rental')) {
            $form.find('[data-type="tab"][data-caption="Rental"]').hide();
        }
        if (!FwFormField.getValueByDataField($form, 'SubRent')) {
            $form.find('[data-type="tab"][data-caption="Sub-Rental"]').hide();
        }
        if (!FwFormField.getValueByDataField($form, 'Repair')) {
    ...
    activityCheckboxEvents($form, mode) {
        const rentalTab = $form.find('[data-type="tab"][data-caption="Rental"]'), subrentalTab = $form.find('[data-type="tab"][data-caption="Sub-Rental"]'), repairTab = $form.find('[data-type="tab"][data-caption="Repair"]');
    ...
    ;
    disableCheckboxesOnLoad($form) {
        if (FwFormField.getValueByDataField($form, 'HasRentalItem')) {
            FwFormField.disable(FwFormField.getDataField($form, 'Rental'));
        }
    }
    dynamicColumns($form) {
        const POTYPE = FwFormField.getValueByDataField($form, "PoTypeId"), $rentalGrid = $form.find('.rentalgrid [data-name="OrderItemGrid"]'), $salesGrid = $form.find('.salesgrid [data-name="OrderItemGrid"]'), $partGrid = $form.find('.partgrid [data-name="OrderItemGrid"]'), $laborGrid = $form.find('.laborgrid [data-name="OrderItemGrid"]'), $miscGrid = $form.find('.miscgrid [data-name="OrderItemGrid"]'), $subRentalGrid = $form.find('.subrentalgrid [data-name="OrderItemGrid"]'), $subSaleGrid = $form.find('.subsalesgrid [data-name="OrderItemGrid"]'), $subLaborGrid = $form.find('.sublaborgrid [data-name="OrderItemGrid"]'), $subMiscGrid = $form.find('.submiscgrid [data-name="OrderItemGrid"]'), fields = jQuery($rentalGrid).find('thead tr.fieldnames > td.column > div.field');
        let fieldNames = [];
        ...
        FwAppData.apiMethod(true, 'GET', `api/v1/potype/${POTYPE}`, null, FwServices.defaultTimeout, function onSuccess(response) {
            let hiddenPurchase = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.PurchaseShowFields));
            let hiddenMisc = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.MiscShowFields));
            let hiddenLabor = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.LaborShowFields));
            let hiddenSubRental = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.SubRentalShowFields));
            let hiddenSubSale = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.SubSaleShowFields));
            let hiddenSubMisc = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.SubMiscShowFields));
            let hiddenSubLabor = fieldNames.filter(function (field) { return !this.has(field); }, new Set(response.SubLaborShowFields));
            ...
            for (let i = 0; i < hiddenSubSale.length; i++) {
                jQuery($subSaleGrid.find(`[data-mappedfield="${hiddenSubSale[i]}"]`)).parent().hide();
            }
            for (let i = 0; i < hiddenSubRental.length; i++) {
                jQuery($subRentalGrid.find(`[data-mappedfield="${hiddenSubRental[i]}"]`)).parent().hide();
            }
            for (let i = 0; i < hiddenSubLabor.length; i++) {
                jQuery($subLaborGrid.find(`[data-mappedfield="${hiddenSubLabor[i]}"]`)).parent().hide();
            }
            for (let i = 0; i < hiddenSubMisc.length; i++) {
                jQuery($subMiscGrid.find(`[data-mappedfield="${hiddenSubMisc[i]}"]`)).parent().hide();
            }
        }, null, null);
    }
    ;
