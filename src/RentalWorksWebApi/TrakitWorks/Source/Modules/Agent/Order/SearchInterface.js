class SearchInterface {
    renderSearchPopup($form, id, type, gridInventoryType) {
        ...
        if (typeof gridInventoryType == 'undefined') {
            ...
            if (gridInventoryType == 'Miscellaneous')
    ...
    populateTypeMenu($popup) {
        ...
        FwAppData.apiMethod(true, 'POST', `api/v1/${categoryType}/browse`, inventoryTypeRequest, FwServices.defaultTimeout, function onSuccess(response) {
            let inventoryTypeIndex, inventoryTypeIdIndex;
            switch (categoryType) {
                case 'misccategory':
    ...
    renderInventory($popup, response) {
        ...
        let descriptionIndex = response.ColumnIndex.Description, quantityAvailable = response.ColumnIndex.QuantityAvailable, conflictDate = response.ColumnIndex.ConflictDate, quantityIn = response.ColumnIndex.QuantityIn, quantityQcRequired = response.ColumnIndex.QuantityQcRequired, quantity = response.ColumnIndex.Quantity, inventoryId = response.ColumnIndex.InventoryId, thumbnail = response.ColumnIndex.Thumbnail, appImageId = response.ColumnIndex.ImageId, classificationIndex = response.ColumnIndex.Classification, classificationColor = response.ColumnIndex.ClassificationColor, classificationDescription = response.ColumnIndex.ClassificationDescription, typeIndex = response.ColumnIndex.InventoryType, categoryIndex = response.ColumnIndex.Category, subCategoryIndex = response.ColumnIndex.SubCategory, availabilityStateIndex = response.ColumnIndex.AvailabilityState, qtyIsStaleIndex = response.ColumnIndex.QuantityAvailableIsStale;
    ...
    events($popup, $form, id) {
        ...
        $popup
            ...
            switch (categoryType) {
                case 'misccategory':
                    typeRequest.uniqueids = { MiscTypeId: $this.attr('data-value') };
                    break;
                case 'laborcategory':
                    typeRequest.uniqueids = { LaborTypeId: $this.attr('data-value') };
                    break;
                default:
                    typeRequest.uniqueids = { InventoryTypeId: $this.attr('data-value') };
                    break;
            }
        ...
        $popup
            .on('click', '.options .optionsbutton', (event) => {
            try {
        ...
        $popup.on('click', '.addToOrder', e => {
            ...
            function addToOrder() {
                ...
                FwAppData.apiMethod(true, 'POST', "api/v1/inventorysearch/addto", request, FwServices.defaultTimeout, function onSuccess(response) {
                    ...
                    let $combinedGrid = $form.find('.combinedgrid [data-name="OrderItemGrid"]'), $orderItemGridRental = $form.find('.rentalgrid [data-name="OrderItemGrid"]'), $quoteItemGrid = $form.find('div[data-name="QuoteItemGrid"]');
        ...
        $popup.on('click', '.invviewbtn', e => {
            let $this = jQuery(e.currentTarget), view = $this.attr('data-buttonview');
        ...
        $popup.on('click', '.incrementQuantity, .decrementQuantity', e => {
            e.stopPropagation();
            const $button = jQuery(e.currentTarget), oldValue = $button.parent().find("input").val();
            if ($button.text() == "+") {
        ...
        if (jQuery('html').hasClass('desktop')) {
            ...
            $popup
                ...
                const $button = jQuery(e.currentTarget), oldValue = $button.parent().find("input").val(), $input = $button.parent().find("input");
                if ($button.text() == "+") {
                ...
                const $button = jQuery(e.currentTarget), $input = $button.parent().find("input");
    ...
    updatePreviewTabQuantity($popup, id, initialLoad) {
        FwAppData.apiMethod(true, 'GET', `api/v1/inventorysearch/gettotal/${id}`, null, FwServices.defaultTimeout, response => {
